...
{
    public class BinariesService : IBinariesService
    {
        public IBinariesRepository _binariesRepository { get; set; }
        public IMenuService _menuService { get; set; }
        public BinariesService(IBinariesRepository binariesRepository, IMenuService menuService)
        {
            _binariesRepository = binariesRepository;
            _menuService = menuService;
        }

        public MyHttpResponseMessage SourceDatabaseDDL(Binaries model)
        {
            return _binariesRepository.SourceDatabaseDDL(model);
        }

        public MyHttpResponseMessage DestinationDatabaseDDL(Binaries model)
        {
            return _binariesRepository.DestinationDatabaseDDL(model);
        }

        public MyHttpResponseMessage GetDatabaseObjects(Binaries model)
        {
            return _binariesRepository.GetDatabaseObjects(model);
        }
