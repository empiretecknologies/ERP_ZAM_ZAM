...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class MerchantPurchaseOrderDetailController : BaseController
    {
        public IMerchantPurchaseOrderDetailService _merchantPurchaseOrderDetailService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public MerchantPurchaseOrderDetailController(IMerchantPurchaseOrderDetailService MerchantPurchaseOrderDetailService, IMenuService menuService, IWebHostEnvironment hostingEnvironment,IBaseService baseService) : base(menuService,baseService)
        {
            _merchantPurchaseOrderDetailService = MerchantPurchaseOrderDetailService;
        ...
        [HttpGet]
        public JsonResult GetMerchantPurchaseOrderDetails()
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult Save(CustomMerchantPurchaseOrderDetail modelRecord)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.Save(modelRecord, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetMerchantPurchaseOrderDetailByCode(int code)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.GetMerchantPurchaseOrderDetailByCode(code, CommonHelper.GetValues(HttpContext));
                var detailData = _merchantPurchaseOrderDetailService.GetMerchantPurchaseOrderDetailDetailByCode(code, CommonHelper.GetValues(HttpContext));
        ...
        [HttpGet]
        public JsonResult GetMerchantPurchaseOrderDetailDetailByCode(int code)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.GetMerchantPurchaseOrderDetailDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetBatchDetailByProcess(string process)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.GetBatchDetailByProcess(process, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int code)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.Delete(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult CopyRecord(CopyRecord record)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.CopyRecord(record, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult DeleteMerchantPurchaseOrderDetailDetailByCode(int code)
        {
            try
            {
                var data = _merchantPurchaseOrderDetailService.DeleteMerchantPurchaseOrderDetailDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetPrintReport(MerchantPurchaseOrderDetailRDLCReport model)
        {
        ...
        private string GenerateReport(MerchantPurchaseOrderDetailRDLCReport model)
        {
            var filePath = "";
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _merchantPurchaseOrderDetailService.GetDataForReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
                    if (responseMessage.msgType != 1)
                    {
                        return "";
                    }
                    var reportData = (CustomMerchantPurchaseOrderDetailForPrintReport)responseMessage.data;
                    ...
                    using (LocalReport report = new LocalReport())
                    {
                        ...
                        using (var stReader = new StreamReader(path))
                        {
                            ...
                            using (var stream = new MemoryStream(byteArray))
                            {
                                ...
                                byte[] file;
                                string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, @"Client\MerchantPurchaseOrderDetail");
                                ...
                                if (report.IsReadyForRendering)
                                {
                                    ...
                                    filePath = $"/Client/MerchantPurchaseOrderDetail/{filePath}";
                                }
                            }
                        }
                    }
        ...
        public async Task<IActionResult> SaveImage()
            ...
            try
            {
                ...
                {
                    string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, "Client", "uploads", "MerchantPurchaseOrderDetail");
                    ...
                    response.msgType = 1;
                    response.data = $"/Client/uploads/MerchantPurchaseOrderDetail/{uniqueFileName}"; // Return file path
                }
                else
                {
