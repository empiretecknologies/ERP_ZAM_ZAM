...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class MpoTrimsController : BaseController
    {
        public IMpoTrimsService _mpoTrimsService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public MpoTrimsController(IMpoTrimsService mpoTrimsService, IMenuService menuService, IWebHostEnvironment hostingEnvironment,IBaseService baseService) : base(menuService,baseService)
        {
            _mpoTrimsService = mpoTrimsService;
        ...
        public JsonResult QuickSearch()
        {
            try
            {
                var data = _mpoTrimsService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult Save(CustomMpoTrims modelRecord)
        {
            try
            {
                var data = _mpoTrimsService.Save(modelRecord, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetMpoTrimsByCode(int code)
        {
            try
            {
                var data = _mpoTrimsService.GetMpoTrimsByCode(code, CommonHelper.GetValues(HttpContext));
                var detailData = _mpoTrimsService.GetMpoTrimsDetailByCode(code, CommonHelper.GetValues(HttpContext));
        ...
        [HttpGet]
        public JsonResult GetMpoTrimsDetailByCode(int code)
        {
            try
            {
                var data = _mpoTrimsService.GetMpoTrimsDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetBatchDetailByProcess(string process)
        {
            try
            {
                var data = _mpoTrimsService.GetBatchDetailByProcess(process, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int code)
        {
            try
            {
                var data = _mpoTrimsService.Delete(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult CopyRecord(CopyRecord record)
        {
            try
            {
                var data = _mpoTrimsService.CopyRecord(record, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult DeleteMpoTrimsDetailByCode(int code)
        {
            try
            {
                var data = _mpoTrimsService.DeleteMpoTrimsDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetPrintReport(MpoTrimsRDLCReport model)
        {
        ...
        private string GenerateReport(MpoTrimsRDLCReport model)
        {
            var filePath = "";
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _mpoTrimsService.GetDataForReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
                    if (responseMessage.msgType != 1)
                    {
                        return "";
                    }
                    var reportData = (CustomMpoTrimsForPrintReport)responseMessage.data;
                    ...
                    using (LocalReport report = new LocalReport())
                    {
                        ...
                        using (var stReader = new StreamReader(path))
                        {
                            ...
                            using (var stream = new MemoryStream(byteArray))
                            {
                                ...
                                byte[] file;
                                string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, @"Client\MpoTrims");
                                ...
                                if (report.IsReadyForRendering)
                                {
                                    ...
                                    filePath = $"/Client/MpoTrims/{filePath}";
                                }
                            }
                        }
                    }
        ...
        public async Task<IActionResult> SaveImage()
            ...
            try
            {
                ...
                {
                    string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, "Client", "uploads", "MpoTrims");
                    ...
                    response.msgType = 1;
                    response.data = $"/Client/uploads/MpoTrims/{uniqueFileName}"; // Return file path
                }
                else
                {
