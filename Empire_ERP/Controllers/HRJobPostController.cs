...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class HRJobPostController : BaseController
    {
        public IHRJobPostService _HRJobPostService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public IPeriodService _periodService { get; set; }
        public HRJobPostController(IHRJobPostService chartOfAccountService, IMenuService menuService, IWebHostEnvironment hostingEnvironment, IPeriodService periodService,IBaseService baseService) : base(menuService,baseService)
        {
            _HRJobPostService = chartOfAccountService;
        ...
        {
            var data = _HRJobPostService.GetHRJobPosts(CommonHelper.GetValues(HttpContext));
            return Json(data);
        }
        ...
        {
            var data = _HRJobPostService.GetEmpMails(CommonHelper.GetValues(HttpContext));
            return Json(data);
        }
        ...
        public JsonResult GetAccountsForTreeView()
        {
            try
            {
                var data = _HRJobPostService.GetAccountsForTreeView(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult QuickSearch()
        {
            try
            {
                var data = _HRJobPostService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        {
            var data = _HRJobPostService.GetHRJobPostById(id, CommonHelper.GetValues(HttpContext));
            return Json(data);
        }
        ...
        public JsonResult GenerateCardNo()
        {
            try
            {
                var data = _HRJobPostService.GenerateCardNo(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult Save(HRJobPost model)
        {
            try
            {
                var data = _HRJobPostService.Save(model, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                var data = _HRJobPostService.Delete(id, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        private string GenerateReport(HRJobPostReport model)
        {
            ...
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _HRJobPostService.GetDataForReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
