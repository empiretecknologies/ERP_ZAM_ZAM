...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class StockReceiveController : BaseController
    {
        public IStockReceiveService _stockReceiveService { get; set; }
        public ICommonService _commonService { get; set; }
        public IBranchService _branchService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public StockReceiveController(IStockReceiveService stockReceiveService, IBranchService branchService, ICommonService commonService, IWebHostEnvironment hostingEnvironment, IMenuService menuService,IBaseService baseService) : base(menuService,baseService)
        {
        ...
        [HttpGet]
        public JsonResult GetStockReceive()
        {
            try
            {
                var data = _stockReceiveService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetStockTransferByCode(int code)
        {
            try
            {
                var data = _stockReceiveService.GetStockReceiveByCode(code, CommonHelper.GetValues(HttpContext));
                var detailData = _stockReceiveService.GetStockReceiveDetailByCode(code, CommonHelper.GetValues(HttpContext));
        ...
        [HttpGet]
        public JsonResult GetStockTransferDetailByCode(int code)
        {
            try
            {
                var data = _stockReceiveService.GetStockReceiveDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult Save(CustomStockReceive modelRecord)
        {
            try
            {
                var data = _stockReceiveService.Save(modelRecord, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int code)
        {
            try
            {
                var data = _stockReceiveService.Delete(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult CopyRecord(CopyRecord record)
        {
            try
            {
                var data = _stockReceiveService.CopyRecord(record, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult DeleteStockTransferDetailByCode(int code)
        {
            try
            {
                var data = _stockReceiveService.DeleteStockReceiveDetailByCode(code, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult GetStockTransferDetailByItem(int code, int qty)
        {
            try
            {
                var data = _stockReceiveService.GetStockReceiveDetailByItem(code, qty, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetReport(List<StockReceiveStickerPrint> data)
        {
        ...
        private byte[] GenerateReport(List<StockReceiveStickerPrint> stockData)
        {
            var filePath = "";
            try
            {
                ...
                {
                    var responseMessage = _stockReceiveService.GetDataForCartonSticker(stockData);
                    ...
                    using (LocalReport report = new LocalReport())
                    {
                        ...
                        using (var stReader = new StreamReader(path))
                        {
                            ...
                            using (var stream = new MemoryStream(byteArray))
                            {
                                ...
                                var reportData = (List<StockTransferStickerPrint>)responseMessage.data;
        ...
        private string GenerateReport(RDLCReport model)
        {
            ...
            try
            {
                if (model != null && model.TRAN_ID > 0 && model.MD_ID > 0)
                {
                    ...
                    var responseMessage = _stockReceiveService.GetDataForPrintReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
                    if (responseMessage.msgType != 1)
                    {
                        return "";
                    }
                    var reportData = (CustomStockTransferForPrintReport)responseMessage.data;
                    Reports.Datasets.BarcodeReportDataset.StockReportDataTable data = new Reports.Datasets.BarcodeReportDataset.StockReportDataTable();
                    using (LocalReport report = new LocalReport())
                    {
        ...
        public JsonResult UpdatePrintStatus(string codes)
        {
            try
            {
                var data = _stockReceiveService.UpdatePrintStatus(codes, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetSodaBookFeedingDetailBySodaDate(DateTime? sodaDate, int branchTo)
        {
            try
            {
                var data = _stockReceiveService.GetSodaBookFeedingDetailBySodaDate(sodaDate, branchTo, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult GetBarcodeList()
        {
            try
            {
                var data = _stockReceiveService.GetBarcodeList();
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public JsonResult GetPickData(string pickId)
        {
            try
            {
                var data = _stockReceiveService.GetPickData(pickId, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
