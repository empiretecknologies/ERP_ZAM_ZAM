...
namespace Empire_ERP.Controllers
{
    [CheckSession]
    [ExtractMenuCode]
    public class MachineInfoController : BaseController
    {
        public IMachineInfoService _MachineInfoService { get; set; }
        private readonly IWebHostEnvironment _hostingEnvironment;
        public MachineInfoController(IMachineInfoService chartOfAccountService, IMenuService menuService, IWebHostEnvironment hostingEnvironment,IBaseService baseService) : base(menuService,baseService)
        {
            _MachineInfoService = chartOfAccountService;
        ...
        public JsonResult QuickSearch()
        {
            try
            {
                var data = _MachineInfoService.QuickSearch(CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet]
        public JsonResult MachineInfoByid(int id)
        {
            var data = _MachineInfoService.GetMachineInfoById(id, CommonHelper.GetValues(HttpContext));
            return Json(data);
        }
        ...
        [HttpPost]
        public JsonResult Save(MachineInfo model)
        {
            try
            {
                var data = _MachineInfoService.Save(model, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Delete(int id)
        {
            try
            {
                var data = _MachineInfoService.Delete(id, CommonHelper.GetValues(HttpContext));
                return Json(data);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        //public JsonResult GetPrintReport(MachineInfoReport model)
        //{
        ...
        //private byte[] GenerateReport(MachineInfoReport model)
        //{
        //    var filePath = "";
        //    try
        //    {
        ...
        //            var responseMessage = _MachineInfoService.GetDataForReport(model, reportDetails, CommonHelper.GetValues(HttpContext));
        //            if (responseMessage.msgType != 1)
        //            {
        //                return null;
        //            }
        //            var reportData = (MachineInfoReport)responseMessage.data;
        ...
        public async Task<IActionResult> SaveImage(string imageName)
        {
            ...
            try
            {
                ...
                {
                    string uploadsFolder = Path.Combine(_hostingEnvironment.WebRootPath, "Client", $"MachineInfo");
                ...
                response.msgType = 1;
                response.data = $"/Client/MachineInfo/{uniqueFileName}";
            }
            catch (Exception ex)
            {
                string _catchMessage = ex.Message;
