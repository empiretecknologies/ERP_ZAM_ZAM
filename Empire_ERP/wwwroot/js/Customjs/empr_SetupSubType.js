var empr_SetupSubType = {
    initEvents: function () {

        $(document).ready(function () {

            empr_SetupSubType.InitQuickSearch();
            ...
            $('#BtnSave').click(function () {
                if (Permissions != "Admin") {
                    if (!$("#Code").val() && !Permissions.r_ADD) {
                        ...
                        if (empr_SetupSubType.validateForm()) {
                            empr_SetupSubType.saveAttempt();
                        }
                    }
                } else {
                    if (empr_SetupSubType.validateForm()) {
            ...
            $('body').on('click', '.elm_edit', function () {
                ...
                empr_SetupSubType.GetSetupSubTypeByID(reportid);
            });

            $('body').on('click', '#BtnNew', function () {
                $('#BtnDelete').hide();
                $('#BtnNew').hide();
                empr_SetupSubType.resetForm();
            });

            $('#BtnDelete').click(function () {
                empr_SetupSubType.DeleteRecord();
            });
    ...
    DeleteRecord: function () {
        ...
        swal({
            ...
            ajaxHelper.ajaxPostJsonData({ id: $('#Code').val() }, "/SetupSubType/Delete", function (data) {
    ...
    saveAttempt: function () {
        ...
        var obj = empr_SetupSubType.GetDataToSave();
        ajaxHelper.ajaxPostJsonData(obj, "/SetupSubType/save", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                empr_SetupSubType.resetForm();
                empr_SetupSubType.InitQuickSearch();
    ...
    InitQuickSearch: function () {
        empr_SetupSubType.GetAllSetupSubTypes();
    },
    GetAllSetupSubTypes: function () {
        ajaxHelper.ajaxGetJson('/SetupSubType/QuickSearch', function (data) {
            empr_SetupSubType.CreateGrid(data.data);
        }, false, true);
    },
    GetSetupSubTypeByID: function (id) {
        ajaxHelper.ajaxGetJson('/SetupSubType/GetSetupSubTypeByID?id=' + id, function (data) {
            empr_SetupSubType.resetForm();
            if (data.msgType == 1) {
