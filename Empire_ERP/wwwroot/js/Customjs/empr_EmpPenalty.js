var empr_EmpPenalty = {
    ...
    initEvents: function () {
        ...
        $(document).ready(function () {
            ...
            //empr_EmpPenalty.InitQuickSearch();

            /*empr_EmpPenalty.InitDDLEducation();*/
            empr_EmpPenalty.InitAccountGroupGridBox();
            /*empr_EmpPenalty.InitHRTables();*/
            empr_EmpPenalty.FieldHideAndShow('');
            empr_EmpPenalty.resetForm();
            ...
            $('#BtnSave').click(function () {
                if (Permissions != "Admin") {
                    if (!$("#Code").val() && !Permissions.r_ADD) {
                        ...
                        if (empr_EmpPenalty.validateForm()) {
                            empr_EmpPenalty.saveAttempt();
                        }
                    }
                } else {
                    if (empr_EmpPenalty.validateForm()) {
            ...
            $('#BtnNew').click(function () {
                ...
                $('.Record').show();
                //empr_EmpPenalty.CreateGrid([{ priority: 'N' }]);
                empr_EmpPenalty.InitQuickSearchGrid();
            });
            ...
            $('body').on('click', '.elm_edit', function () {
                ...
                empr_EmpPenalty.GetEmpPenaltyByID(reportid, empr_EmpPenalty.TableName);
            });
    ...
    InitQuickSearchGrid: function () {
        empr_EmpPenalty.GetPurchaseOrders();
    },

    GetPurchaseOrders: function () {
        debugger;
        var EmpId = parseInt($("#ENAME").dxDropDownBox("instance").option("value")) || 0;
        ajaxHelper.ajaxGetJson('/EmpPenalty/GetEmpPenaltyRecord?id=' + EmpId, function (data) {
            console.log(data);
            if (data.msgType == 1) {
                if (data.data.length > 0) {
                    empr_EmpPenalty.CreateGrid(data.data);
                }
                else {
                    empr_EmpPenalty.CreateGrid([{ priority: 'N' }]);
                }
    ...
    CreateGrid: function (dataSrc) {
        if (dataSrc.length > 0) {
            empr_EmpPenalty.rowsCount = dataSrc.length - 1;
        ...
        var col = [
            {
                ...
                cellTemplate: function (container, options) {
                    $(`<div class="btn-group btn-group-sm">
                            <a href="javascript:;" class="grid-action-icon" style="margin-left: 8px" onclick="empr_EmpPenalty.AddRow()" title="Add"><i class="fa fa-add"></i></a>
                            <a href="javascript:;" class="grid-action-icon Delete" style="margin-left: 8px" onclick="empr_EmpPenalty.DeleteRow(${options.rowIndex},${options.data.dT_CODE})" title="Delete"><i class="fa fa-trash"></i></a>
        ...
        if (dataSource.length > 0) {
            ...
            if (traN_ID == '' || traN_ID == null || traN_ID == undefined) {
                ...
                swal({
                    ...
                    ajaxHelper.ajaxGetJson('/EmpPenalty/Delete?id=' + traN_ID, function (data) {
                        empr_helper.notify(data.msg, data.msgType);
                        if (data.msgType == 1) {
                            gridInstance.deleteRow(index);
                            empr_EmpPenalty.rowsCount -= 1;
    ...
    DeleteRecord: function () {
        ...
        swal({
            ...
            ajaxHelper.ajaxPostJsonData({ id: $('#Code').val(), TableName: empr_EmpPenalty.TableName }, "/EmpPenalty/Delete", function (data) {
                empr_helper.notify(data.msg, data.msgType);
                if (data.msgType == 1) {
                    empr_EmpPenalty.resetForm();
                    empr_EmpPenalty.InitQuickSearch(empr_EmpPenalty.TableName);
                    $('#optmodal').modal('hide');
                    $('#BtnDelete').hide();
                    $('#BtnNew').hide();
                }
    ...
    InitAccountGroupGridBox: function (_selectedValue) {
        ...
        $.ajax({
            url: "EmpPenalty/GetEmployeeGroups",
            type: "GET",
    ...
    saveAttempt: function () {
        debugger;
        var obj = empr_EmpPenalty.GetDataToSave();
        console.log(obj);
        ajaxHelper.ajaxPostJsonData(obj, "/EmpPenalty/save", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                empr_EmpPenalty.resetForm();
                empr_EmpPenalty.InitQuickSearch(empr_EmpPenalty.TableName);
    ...
    InitQuickSearch: function (TableName) {
        empr_EmpPenalty.GetAllEmpPenaltys(TableName);
    },
    InitHRTables: function (selectedValue) {
        console.log(EmpPenaltyTables);
        $('#TNAME').dxSelectBox({
            dataSource: EmpPenaltyTables,
            displayExpr: 'value',
            valueExpr: 'key',
            value: selectedValue,
            searchEnabled: true,
            ...
            onValueChanged: function (e) {
                ...
                var selectedItem = EmpPenaltyTables.find(x => x.key === selectedKey);
                empr_EmpPenalty.TableName = selectedItem.tname;
                empr_EmpPenalty.MasterId = selectedItem.key;
            }
        });
    },
    ...
    GetAllEmpPenaltys: function (TableName) {
        ajaxHelper.ajaxGetJson('/EmpPenalty/QuickSearch?TableName=' + TableName, function (data) {
            empr_EmpPenalty.CreateGrid(data.data, TableName);
        }, false, true);
    },
    GetEmpPenaltyByID: function (id, tableName) {
        ajaxHelper.ajaxGetJson('/EmpPenalty/GetEmpPenaltyByID?id=' + id + '&TableName=' + tableName, function (data) {
            empr_EmpPenalty.resetForm();
            if (data.msgType == 1) {
                ...
                empr_EmpPenalty.InitAccountGroupGridBox(record.emP_ID);
    ...
    AddRow: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                dataSource.unshift({ __KEY__: empr_EmpPenalty.GenerateKey(36), priority: 'N' });
