var empr_DocumentRetrieval = {
    totalCount: 0,
    rowsCount: 0,
    InitEvents: function () {
        $(document).ready(function () {
            empr_DocumentRetrieval.InitQuickSearchGrid();
            empr_DocumentRetrieval.ResetForm();
            window.addEventListener('message', function (event) {
                ...
                if (data && data.traN_ID) {
                    ...
                    empr_DocumentRetrieval.GetDocumentRetrievalByCode(data.traN_ID);
                }
            });
            $('body').on('click', '.elm_edit', function () {
                ...
                $('.vHide').show();
                $('.modal').modal('hide');
                empr_DocumentRetrieval.GetDocumentRetrievalByCode(id);
            });
            ...
            $('body').on('click', '#BtnSave', function () {
                if (Permissions != "Admin") {
                    if (!$("#Code").val() && !Permissions.r_ADD) {
                        ...
                        if (empr_DocumentRetrieval.ValidateMainInfo()) {
                            empr_DocumentRetrieval.Save();
                        }
                    }
                } else {
                    if (empr_DocumentRetrieval.ValidateMainInfo()) {
            ...
            $('body').on('click', '#BtnSodaPick', function () {
                empr_DocumentRetrieval.InitSodaPickGrid();
            });
            ...
            $('body').on('click', '#BtnAddSodaToDelivery', function () {
                ...
                if (selectedSodas.length > 0) {
                    empr_DocumentRetrieval.AddSodaToDelivery();
                }
                else {
    ...
    CreateGrid: function (dataSrc) {
        if (dataSrc.length > 0) {
            empr_DocumentRetrieval.rowsCount = dataSrc.length - 1;
        ...
        var col = [
                ...
                cellTemplate: function (container, options) {
                    if (Permissions != "Admin") {
                        const copyAction = !Permissions.r_COPY
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Clone" onclick="empr_DocumentRetrieval.CloneRow(${options.rowIndex})" title="Duplicate"><i class="fa fa-clone"></i></a>`;
                        const addAction = (!Permissions.r_ADD && !Permissions.r_EDIT)
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Add" style="margin-left: 8px" onclick="empr_DocumentRetrieval.AddRow()" title="Add"><i class="fa fa-add"></i></a>`;
                        const deleteAction = !Permissions.r_DLT
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Delete" style="margin-left: 8px" onclick="empr_DocumentRetrieval.DeleteRow(${options.rowIndex},${options.data.dT_CODE})" title="Delete"><i class="fa fa-trash"></i></a>`;
                        const actions = `<div class="btn-group btn-group-sm">${copyAction}${addAction}${deleteAction}</div>`;
                        $(actions).appendTo(container);
                    } else {
                        $(`<div class="btn-group btn-group-sm">
                           <a href="javascript:;" class="grid-action-icon" onclick="empr_DocumentRetrieval.CloneRow(`+ options.rowIndex + `)" title="Duplicate"><i class="fa fa-clone"></i></a>
                           <a href="javascript:;" class="grid-action-icon" style="margin-left: 8px" onclick="empr_DocumentRetrieval.AddRow()" title="Add"><i class="fa fa-add"></i></a>
                           <a href="javascript:;" class="grid-action-icon" style="margin-left: 8px" onclick="empr_DocumentRetrieval.DeleteRow(${options.rowIndex},${options.data.dT_CODE})" title="Delete"><i class="fa fa-trash"></i></a>
                ...
                cellTemplate: function (container, options) {
                    ...
                    fileInput.on('change', function (event) {
                        ...
                        if (file) {
                            ...
                            $.ajax({
                                url: '/DocumentRetrieval/SaveImage',
                                data: formData,
                                processData: false,
                                contentType: false,
                                type: "POST",
                                success: function (data) {
                                    if (data.msgType == '1') {
                                        ...
                                        dataSource[rowIndex].voucher = data.data;
                    ...
                    if (options.data.voucher != null && options.data.voucher !== '') {
                        const viewButton = $('<div>')
                            ...
                            .append(
                                $('<a>')
                                    ...
                                    .on('click', function () {
                                        const fileUrl = options.data.voucher;
                                        if (fileUrl) {
            ...
            },
            {
                ...
                cellTemplate: function (container, options) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if (data.msgType == '1') {
                                        ...
                                        dataSource[rowIndex].swift = data.data;
                                        grid.repaint(); // Ensures the UI updates correctly
                                    } else {
                                        console.error("Upload Error:", data);
                                        empr_helper.notify("Something went wrong while saving the file. Please re-upload.", 2);
                    ...
                    inputGroup.append(fileInput);
                    if (options.data.swift != null && options.data.swift !== '') {
                        const viewButton = $('<div>')
                            .addClass('input-group-append')
                            .append(
                                $('<a>')
                                    ...
                                    .addClass('input-group-text')
                                    .on('click', function () {
                                        const fileUrl = options.data.swift;
                                        if (fileUrl) {
                                            window.open(fileUrl, '_blank');
                                        } else {
                                            alert('No document available to view.');
            ...
            },
            {
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    var selectedItem = CurrencyData.filter(u => u.key == value);
                    if (selectedItem.length > 0) {
                        newData.crate = selectedItem[0].rate;
                    }
                }
            },
            {
            ...
            //},
            {
                ...
                cellTemplate: function (container, options) {
                    $('<a>')
                        ...
                        .text(options.value)
                        .attr('href', '#')
                        .attr('onclick', 'empr_DocumentRetrieval.openVoucherPage(' + JSON.stringify(options.data.link) + ', ' + JSON.stringify(options.data.ptraN_ID) + ')')
                        .appendTo(container);
                }
            },
            {
    ...
    CloneRow: function (index) {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                empr_DocumentRetrieval.rowsCount += 1;
                ...
                if (dataSource.length > 0) {
                    ...
                    //delete clonedRowData.dT_CODE;
                    clonedRowData.__KEY__ = empr_DocumentRetrieval.GenerateKey(36);
    ...
    AddRow: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                dataSource.unshift({ __KEY__: empr_DocumentRetrieval.GenerateKey(36), priority: 'N' });
                ...
                if (dtCode == '' || dtCode == null || dtCode == undefined) {
                    ...
                    if (availableRows.length > 1) {
                        swal({
                            ...
                            ajaxHelper.ajaxPostJsonData({ code: dtCode }, "/DocumentRetrieval/DeleteDocumentRetrievalDetailByCode", function (data) {
    ...
    dropDownBoxEditorTemplate: function (cellElement, cellInfo) {
        return $('<div>').dxDropDownBox({
            ...
            //    //    return $.ajax({
            //    //        url: "/DocumentRetrieval/GetItems",
            //    //        method: "GET"
            //    //    });
            //    //}
            //    load(loadOptions) {
    ...
    InitQuickSearchGrid: function () {
        empr_DocumentRetrieval.GetDocumentRetrievals();
    },

    GetDocumentRetrievals: function () {
        ajaxHelper.ajaxGetJson('/DocumentRetrieval/GetDocumentRetrievals', function (data) {
            if (data.msgType == 1) {
                empr_DocumentRetrieval.CreateQuickSearchGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    CreateQuickSearchGrid: function (dataSrc) {
        ...
        if (dataSrc.length > 0 ? dataSrc[0].picK_DATA != undefined : false) {
            ...
            {
                dataField: 'picK_DATA', caption: 'Pick Data #',
                cellTemplate: function (container, options) {
                    $('<a>')
                        .addClass('dx-link')
                        .text(options.value)
            ...
            empr_helper.dxGridbindingVouchers('#gridContainer', col, dataSrc, "DocumentRetrievalQSD");
        } else {
            var col = [{
                dataField: "Action",
                width: 100,
            ...
            empr_helper.dxGridbindingVouchers('#gridContainer', col, dataSrc, "DocumentRetrievalQS");
        }
        setTimeout(function () {
    ...
    GetDocumentRetrievalByCode: function (code) {
        ajaxHelper.ajaxGetJson('/DocumentRetrieval/GetDocumentRetrievalByCode?code=' + code, function (data) {
            if (data.master.msgType == 1) {
                ...
                if (data.detail.msgType == 1) {
                    empr_DocumentRetrieval.CreateGrid(data.detail.data);
    ...
    GetDocumentRetrievalDetailByCode: function (code) {
        ajaxHelper.ajaxGetJson('/DocumentRetrieval/GetDocumentRetrievalDetailByCode?code=' + code, function (data) {
            if (data.msgType == 1) {
                empr_DocumentRetrieval.CreateGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    GetDataToSave: function () {
        ...
        //    });
        //}
        //if (empr_DocumentRetrieval.rowsCount == detailRecords.length) {
        //    var modelRecord = {
        //        Master: masterRecord,
    ...
    ValidateMainInfo: function () {
        ...
        var valid = true;
        var data = empr_DocumentRetrieval.GetDataToSave();
        console.log(data)
        ...
        $.each(data.Detail, function (index, item) {
            ...
            if (item.voucher == "" || item.voucher == null || item.voucher == undefined) {
                empr_helper.notify("Please Upload voucher Document " + index, 2);
                valid = false;
                return valid;
            }

            if (item.swift == "" || item.swift == null || item.swift == undefined) {
                empr_helper.notify("Please Upload swift Document " + index, 2);
                valid = false;
                return valid;
            }

            if (item.curR_CODE == "" || item.curR_CODE == null || item.curR_CODE == undefined) {
                empr_helper.notify("Please Select Currency" + index, 2);
                valid = false;
                return valid;
            }
    ...
    Save: function () {
        debugger;
        var dataModel = empr_DocumentRetrieval.GetDataToSave();
        debugger;
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/DocumentRetrieval/Save", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                if (dataModel.Master.TRAN_ID == 0
                    || dataModel.Master.TRAN_ID == null
                ...
                empr_DocumentRetrieval.GetDocumentRetrievalDetailByCode($('#Code').val());
                $('#BtnDelete').show();
            }
        }, false, true);
    },
    ...
    ResetForm: function () {
        ...
        empr_DocumentRetrieval.CreateGrid([{ priority: 'N' }]);
    ...
    Delete: function () {
        ...
        swal({
            ...
            ajaxHelper.ajaxPostJsonData({ code: $('#Code').val() }, "/DocumentRetrieval/Delete", function (data) {
                empr_helper.notify(data.msg, data.msgType);
                if (data.msgType == 1) {
                    empr_DocumentRetrieval.ResetForm();
                    $('#BtnDelete').hide();
    ...
    InitSodaPickGrid: function () {
        ajaxHelper.ajaxGetJson('/DocumentRetrieval/GetSodaBookFeedingDetail', function (data) {
            if (data.msgType == 1) {
                if (data.data.length > 0) {
                    if ($('#SodaPickGridContainer').data('dxDataGrid') != undefined) {
                        $('#SodaPickGridContainer').data('dxDataGrid').dispose();
                    }
                    empr_DocumentRetrieval.CreateSodaPickGrid(data.data);
                    $('#SodaPickModal').modal('show');
                } else {
                    empr_helper.notify("No Record found.", 2);
                }
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
    ...
    CreateSodaPickGrid: function (dataSrc) {
        var existingPickIds = empr_DocumentRetrieval.GetAllPickIds();
        ...
        var col = [
            ...
            { dataField: 'orignaL_DOC', caption: 'Orignal Doc', allowEditing: false, },
            { dataField: 'dhL_NO', caption: 'DHL NO', allowEditing: false, },
        ];
        ...
        if ($('#SodaPickGridContainer').dxDataGrid('instance').hasEditData()) {
            $('#SodaPickGridContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                if (IsDataAvailableInGrid) {
                    ...
                    var selectedSodas = $('#SodaPickGridContainer').dxDataGrid('instance').getSelectedRowKeys();
                    //selectedSodas = empr_DocumentRetrieval.SetData(selectedSodas);
