var empr_DailyProduction = {
    ...
    InitEvents: function () {
        $(document).ready(function () {
            empr_DailyProduction.ResetForm();
            empr_DailyProduction.InitReportTypeDDL();

            $('body').on('click', '#BtnQuickSearch', function () {
                empr_DailyProduction.InitQuickSearchGrid();
            });
            ...
            $('body').on('click', '.elm_edit', function () {
                ...
                empr_DailyProduction.GetDailyProductionByCode(id);
            });
            ...
            $('body').on('click', '#saveCopiedRecord', function () {
                ajaxHelper.ajaxPostJsonData({ traN_ID: empr_helper.selectedBill, v_DATE: $('#updatedDate').val() }, "/DailyProduction/CopyRecord", function (data) {
                    console.log(data.data);
                    empr_helper.notify(data.msg, data.msgType);
                    if (data.msgType == 1) {
                        $('.modal').modal('hide');
                        empr_DailyProduction.GetDailyProductionByCode(data.data.code);
                    }
                }, false, true);
            });
            ...
            $('body').on('click', '#BtnSave', function () {
                if (Permissions != "Admin") {
                    if (!$("#Code").val() && !Permissions.r_ADD) {
                        ...
                        if (empr_DailyProduction.ValidateInfo()) {
                            empr_DailyProduction.SaveInfo();
                        }
                    }
                } else {
                    if (empr_DailyProduction.ValidateInfo()) {
            ...
            $('body').on('click', '#BtnDelete', function () {
                empr_DailyProduction.Delete();
            });

            $('body').on('click', '#BtnPrint, #BtnGenerateReport', function () {
                empr_DailyProduction.GeneratePrintReport();
            });
            ...
            $('body').on('click', '#BtnBatchPick', function () {
                empr_DailyProduction.InitBatchPickGrid();
            });
            ...
            $('body').on('click', '#BtnAddBatch', function () {
                ...
                if (selectedSodas.length > 0) {
                    empr_DailyProduction.AddBatchToDailyProduction();
                }
                else {
    ...
    //        MD_ID: MD_ID,
    //    }
    //    ajaxHelper.ajaxPostJsonData(dataModel, "/DailyProduction/GetPrintReport", function (data) {
    //        if (data.msgType == 1) {
    //            $('#ModalBody').empty();
    //            setTimeout(function () {
    ...
    InitReportTypeDDL: function (selectedValue) {
        ajaxHelper.ajaxGetJson("/DailyProduction/GetReportTypes", function (data) {
            if (data.msgType == 1) {
    ...
    ResetForm: function () {
        empr_DailyProduction.CreateGrid([{ __KEY__: empr_DailyProduction.GenerateKey(36) }]);
        ...
        empr_DailyProduction.InitFinishItemsDDL();
        empr_DailyProduction.InitProcessesDDL();
        //const mfgInput = document.getElementById('MFG_DATE');
    ...
    CreateGrid: function (dataSrc) {
        console.log(dataSrc)
        if (dataSrc.length > 0) {
            empr_DailyProduction.rowsCount = dataSrc.length - 1;
                ...
                cellTemplate: function (container, options) {
                    if (Permissions != "Admin") {
                        const copyAction = !Permissions.r_COPY
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Clone" onclick="empr_DailyProduction.CloneRow(${options.rowIndex})" title="Duplicate"><i class="fa fa-clone"></i></a>`;
                        const addAction = (!Permissions.r_ADD && !Permissions.r_EDIT)
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Add" style="margin-left: 8px" onclick="empr_DailyProduction.AddRow()" title="Add"><i class="fa fa-add"></i></a>`;
                        const deleteAction = !Permissions.r_DLT
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Delete" style="margin-left: 8px" onclick="empr_DailyProduction.DeleteRow(${options.rowIndex})" title="Delete"><i class="fa fa-trash"></i></a>`;
                        const actions = `<div class="btn-group btn-group-sm">${copyAction}${addAction}${deleteAction}</div>`;
                        $(actions).appendTo(container);
                    } else {
                        $(`<div class="btn-group btn-group-sm">
                           <a href="javascript:;" class="grid-action-icon" onclick="empr_DailyProduction.CloneRow(`+ options.rowIndex + `)" title="Duplicate"><i class="fa fa-clone"></i></a>
                           <a href="javascript:;" class="grid-action-icon" style="margin-left: 8px" onclick="empr_DailyProduction.AddRow()" title="Add"><i class="fa fa-add"></i></a>
                           <a href="javascript:;" class="grid-action-icon" style="margin-left: 8px" onclick="empr_DailyProduction.DeleteRow(`+ options.rowIndex + `)" title="Delete"><i class="fa fa-trash"></i></a>
        ...
        empr_helper.editableDxGridbindingForTransactions('#DetailContainer', col, dataSrc, "DailyProduction", "iteM_CODE", '');
        if (dataSrc.length == 0) {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                if (dataSource.length > 0) {
                    ...
                    //delete clonedRowData.dT_CODE;
                    clonedRowData.__KEY__ = empr_DailyProduction.GenerateKey(36);
    ...
    AddRow: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                dataSource.unshift({ __KEY__: empr_DailyProduction.GenerateKey(36) });
                gridInstance.option("dataSource", dataSource);
            ...
            if (dataSource.length > 1) {
                ...
                if (row.dT_CODE == '' || row.dT_CODE == null || row.dT_CODE == undefined) {
                    ...
                    swal({
                        ...
                        ajaxHelper.ajaxPostJsonData({ code: row.dT_CODE }, "/DailyProduction/DeleteDailyProductionDetailByCode", function (data) {
                            empr_helper.notify(data.msg, data.msgType);
                            if (data.msgType == 1) {
                                gridInstance.deleteRow(index);
                                empr_DailyProduction.rowsCount -= 1;
    ...
    InitFinishItemsDDL: function (selectedValue) {
        $.ajax({
            ...
            success: function (response) {
                empr_DailyProduction.BindDxDDL("FinishItem", response.data, selectedValue, "key", "value", "Select", function (d) {
    ...
    InitProcessesDDL: function (selectedValue) {
        $.ajax({
            url: "DailyProduction/GetProcesses",
            type: "GET",
            success: function (response) {
                empr_DailyProduction.BindDxDDL("Process", response.data, selectedValue, "key", "value", "Select", function (d) {
    ...
    InitQuickSearchGrid: function () {
        empr_DailyProduction.GetDailyProductions();
    },
    GetDailyProductions: function () {
        ajaxHelper.ajaxGetJson('/DailyProduction/GetDailyProductions', function (data) {
            if (data.msgType == 1) {
                empr_DailyProduction.CreateQuickSearchGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    GetDataToSave: function () {
        ...
        if (empr_DailyProduction.rowsCount == detailRecords.length) {
            var modelRecord = {
                Master: masterRecord,
    ...
    ValidateInfo: function () {
        ...
        var valid = true;
        var data = empr_DailyProduction.GetDataToSave();
    ...
    SaveInfo: function () {
        debugger;
        var dataModel = empr_DailyProduction.GetDataToSave();
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/DailyProduction/Save", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                if (dataModel.Master.TRAN_ID == 0
                    || dataModel.Master.TRAN_ID == null
                ...
                empr_DailyProduction.GetDailyProductionDetailsByCode($('#Code').val());
                $('#BtnDelete').show();
            }
        }, false, true);
    },
    GetDailyProductionByCode: function (code) {
        ajaxHelper.ajaxGetJson('/DailyProduction/GetDailyProductionByCode?code=' + code, function (data) {
            if (data.master.msgType == 1) {
                ...
                if (masterData.length == 1) {
                    ...
                    //$('#MFG_DATE').val(response.mfgdate); Ammar
                    //$('#EXP_DATE').val(response.expdate);
                    empr_DailyProduction.InitFinishItemsDDL(parseInt(response.iteM_CODE));
                    empr_DailyProduction.InitProcessesDDL(parseInt(response.process));
                    //$('#BtnDelete').show();
                    if (Permissions != "Admin") {
                ...
                if (data.detail.msgType == 1) {
                    empr_DailyProduction.CreateGrid(data.detail.data);
    ...
    GetDailyProductionDetailsByCode: function (code) {
        ajaxHelper.ajaxGetJson('/DailyProduction/GetDailyProductionDetailByCode?code=' + code, function (data) {
            if (data.msgType == 1) {
                empr_DailyProduction.CreateGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    Delete: function () {
        ...
        swal({
            ...
            ajaxHelper.ajaxPostJsonData({ code: $('#Code').val() }, "/DailyProduction/Delete", function (data) {
                empr_helper.notify(data.msg, data.msgType);
                if (data.msgType == 1) {
                    empr_DailyProduction.ResetForm();
                    $('#BtnDelete').hide();
    ...
    InitBatchPickGrid: function () {
        ...
        if (data.Master.PROCESS == "" || data.Master.PROCESS == null || data.Master.PROCESS == undefined) {
            ...
            empr_DailyProduction.GetBatchDetailByProcess(data.Master.PROCESS);
        }
    },

    GetBatchDetailByProcess: function (process) {
        ajaxHelper.ajaxGetJson('/DailyProduction/GetBatchDetailByProcess?process=' + process, function (data) {
            console.log('Pick Grid :',data.data);
            if (data.msgType == 1) {
                if (data.data.length > 0) {
                    if ($('#SodaPickGridContainer').data('dxDataGrid') != undefined) {
                        $('#SodaPickGridContainer').data('dxDataGrid').dispose();
                    }
                    empr_DailyProduction.CreateBatchGrid(data.data);
        ...
        if ($('#SodaPickGridContainer').dxDataGrid('instance').hasEditData()) {
            $('#SodaPickGridContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                if (IsDataAvailableInGrid) {
                    ...
                    var selectedSodas = $('#SodaPickGridContainer').dxDataGrid('instance').getSelectedRowKeys();
                    //selectedSodas = empr_DailyProduction.SetData(selectedSodas);
    ...
    SetData: function (dataSource) {
        $.each(dataSource, function (index, item) {
            ...
            if (!isNaN(qty) && !isNaN(qtY2)) {
                if (item.chK1) {
                    item.baL_QTY = qty * qtY2;
                    if (empr_DailyProduction.Branch_RT_TYPE == 'Y') {
                        item.amt = item.baL_QTY * rate;
                    ...
                    if (empr_DailyProduction.Branch_RT_TYPE == 'N') {
                        item.amt = qty * rate;
                    }
            ...
            item.__KEY__ = empr_DailyProduction.GenerateKey(36);
        });
