var empr_DeliveryFeeding = {
    ...
    InitEvents: function () {
        $(document).ready(function () {
            ...
            $('.coBroker').hide();
            empr_DeliveryFeeding.ResetForm();
            window.addEventListener('message', function (event) {
                ...
                if (data && data.traN_ID) {
                    ...
                    empr_DeliveryFeeding.GetDeliveryFeedingByCode(data.traN_ID);
                }
            });

            $('body').on('click', '#BtnQuickSearch', function () {
                empr_DeliveryFeeding.InitQuickSearchGrid();
            });
            ...
            $('body').on('click', '.elm_print', function () {
                ...
                empr_DeliveryFeeding.GeneratePrintReport();
            });
            ...
            $('body').on('click', '#BtnSave', function () {
                ...
                if (Permissions != "Admin") {
                    if (!$("#Code").val() && !Permissions.r_ADD) {
                        ...
                        if (empr_DeliveryFeeding.ValidateInfo()) {
                            empr_DeliveryFeeding.SaveInfo();
                        }
                    }
                } else {
                    if (empr_DeliveryFeeding.ValidateInfo()) {
            ...
            $('body').on('click', '.elm_edit', function () {
                ...
                empr_DeliveryFeeding.GetDeliveryFeedingByCode(id);
            });
            ...
            $('body').on('click', '#saveCopiedRecord', function () {
                ajaxHelper.ajaxPostJsonData({ traN_ID: empr_helper.selectedBill, v_DATE: $('#updatedDate').val() }, "/DeliveryFeeding/CopyRecord", function (data) {
                    console.log(data.data);
                    empr_helper.notify(data.msg, data.msgType);
                    if (data.msgType == 1) {
                        $('.modal').modal('hide');
                        empr_DeliveryFeeding.GetDeliveryFeedingByCode(data.data.code);
                    }
                }, false, true);
            });

            $('body').on('click', '#BtnDelete', function () {
                empr_DeliveryFeeding.Delete();
            });
            ...
            $('body').on('click', '#BtnSodaPick', function () {
                empr_DeliveryFeeding.InitSodaPickGrid();
            });
            ...
            $('body').on('click', '#BtnAddSodaToDelivery', function () {
                ...
                if (selectedSodas.length > 0) {
                    empr_DeliveryFeeding.AddSodaToDelivery();
                }
                else {
    ...
    ResetForm: function () {
        empr_DeliveryFeeding.CreateGrid([{ __KEY__: empr_DeliveryFeeding.GenerateKey(36), chK1: false, chk: "0" }]);
        empr_DeliveryFeeding.InitTypeDDL();
        empr_DeliveryFeeding.InitSodaTypeDDL();
        empr_DeliveryFeeding.InitUnitDDL();
        empr_DeliveryFeeding.InitCurrencyDDL();
        empr_DeliveryFeeding.InitCoBrokerCodeDDL();
        ...
        empr_DeliveryFeeding.GetDeliveryFeedingDefaultOperators();
        var dataSource = [
            {
                "Action": "",
                "dT_CODE": "abc123",
        ...
        empr_DeliveryFeeding.InitDropdowns();
        empr_DeliveryFeeding.InitReportTypeDDL();
        $('#SDATE').focus();
    },
    CreateGrid: function (dataSrc) {
        console.log('CreateGrid Old Link', dataSrc);
        if (dataSrc.length > 0) {
            empr_DeliveryFeeding.rowsCount = dataSrc.length - 1;
        }
        ...
        var col = [
                ...
                cellTemplate: function (container, options) {
                    if (Permissions != "Admin") {
                        const copyAction = !Permissions.r_COPY
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Clone" onclick="empr_DeliveryFeeding.CloneRow(${options.rowIndex})" title="Duplicate"><i class="fa fa-clone"></i></a>`;
                        const addAction = (!Permissions.r_ADD && !Permissions.r_EDIT)
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Add" style="margin-left: 8px" onclick="empr_DeliveryFeeding.AddRow()" title="Add"><i class="fa fa-add"></i></a>`;
                        const deleteAction = !Permissions.r_DLT
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Delete" style="margin-left: 8px" onclick="empr_DeliveryFeeding.DeleteRow(${options.rowIndex},${options.data.dT_CODE})" title="Delete"><i class="fa fa-trash"></i></a>`;
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    if (!isNaN(qty) && !isNaN(qtY2)) {
                        if (currentRowData.chK1) {
                            ...
                            if (empr_DeliveryFeeding.Branch_RT_TYPE == 'Y') {
                            ...
                            if (empr_DeliveryFeeding.Branch_RT_TYPE == 'N') {
                ...
                cellTemplate: function (container, options) {
                    ...
                    var $checkbox = $("<input type='checkbox'>")
                        ...
                        .on('change', function () {
                            ...
                            var gridInstance = $('#deliveryFeedingDetailContainer').dxDataGrid('instance');
            ...
            },
            {
                ...
                caption: 'RT Type',
                lookup: {
                    dataSource: empr_DeliveryFeeding.RT_TYPES,
                    displayExpr: 'value',
                    valueExpr: 'key'
                },
                editorOptions: {
                    ...
                    onCustomItemCreating: function (args) {
                        if (args.text != undefined) {
                            ...
                            if (isNaN(newItem.key)) {
                                ...
                                empr_DeliveryFeeding.RT_TYPES.insert(newItem);
                                //args.CustomItem = newItem;
                                args.customItem = newItem;
                                setTimeout(function () {
                                    $('#deliveryFeedingDetailContainer').dxDataGrid('instance')
            ...
            //},
            {
                ...
                cellTemplate: function (container, options) {
                    $('<a>')
                        ...
                        .text(options.value)
                        .attr('href', '#')
                        .attr('onclick', 'empr_DeliveryFeeding.openVoucherPage(' + JSON.stringify(options.data.link) + ', ' + JSON.stringify(options.data.traN_ID) + ')')
                        .appendTo(container);
                }
            },
            {
        ...
        empr_helper.editableDxGridbindingForTransactionsVouchers('#deliveryFeedingDetailContainer', col, dataSrc, "DeliveryFeeding", "iteM_CODE");
        if (dataSrc.length == 0) {
            $('#deliveryFeedingDetailContainer').dxDataGrid('instance').addRow().done(function () {
                $('#deliveryFeedingDetailContainer').dxDataGrid('instance').saveEditData();
            });
        }

        //setTimeout(function () {
        //    var nextElement = $('#deliveryFeedingDetailContainer').dxDataGrid('instance').getCellElement(0, 'iteM_CODE');
        //    $('#deliveryFeedingDetailContainer').dxDataGrid('instance').focus(nextElement);  
        //}, 1500);
    },
    CloneRow: function (index) {
        debugger;
        const gridIns = $('#deliveryFeedingDetailContainer').dxDataGrid('instance');
        ...
        if ($('#deliveryFeedingDetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#deliveryFeedingDetailContainer').dxDataGrid('instance').saveEditData().done(function () {

                empr_DeliveryFeeding.rowsCount += 1;
                const gridInstance = $('#deliveryFeedingDetailContainer').dxDataGrid('instance');
                var dataSource = gridInstance.option("dataSource");
                if (dataSource.length > 0) {
                    ...
                    //delete clonedRowData.dT_CODE;
                    clonedRowData.__KEY__ = empr_DeliveryFeeding.GenerateKey(36);
    ...
    AddRow: function () {
        ...
        if ($('#deliveryFeedingDetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#deliveryFeedingDetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                dataSource.unshift({ __KEY__: empr_DeliveryFeeding.GenerateKey(36), chK1: true, chk: "1" });
                gridInstance.option("dataSource", dataSource);
                ...
                if (dtCode == '' || dtCode == null || dtCode == undefined) {
                    ...
                    if (availableRows.length > 1) {
                        swal({
                            ...
                            ajaxHelper.ajaxPostJsonData({ code: dtCode }, "/DeliveryFeeding/DeleteDeliveryFeedingDetailByCode", function (data) {
                                empr_helper.notify(data.msg, data.msgType);
                                if (data.msgType == 1) {
                                    gridInstance.deleteRow(index);
                                    empr_DeliveryFeeding.rowsCount -= 1;
    ...
    InitQuickSearchGrid: function () {
        empr_DeliveryFeeding.GetDeliveryFeedings();
    },
    GetDeliveryFeedings: function () {
        ajaxHelper.ajaxGetJson('/DeliveryFeeding/GetDeliveryFeedings', function (data) {
            if (data.msgType == 1) {
                empr_DeliveryFeeding.CreateQuickSearchGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    CreateQuickSearchGrid: function (dataSrc) {
            ...
            //{ dataField: 'sodA_VOUCHER', caption: 'Soda No', },
            {
                ...
                cellTemplate: function (container, options) {
                    $('<a>')
                        ...
                        .text(options.value)
                        .attr('href', '#')
                        .attr('onclick', 'empr_DeliveryFeeding.openVoucherPage(' + JSON.stringify(options.data.link) + ', ' + JSON.stringify(options.data.ptraN_ID) + ')')
                        .appendTo(container);
                }
            },
        ...
        //empr_helper.dxGridbindingLazyLoading('#gridContainer', columns, "/DeliveryFeeding/GetDeliveryFeeding", "traN_ID", "DeliveryFeeding");
        //var detailColumns = [
    ...
    GetDataToSave: function () {
        ...
        var totalAmount = $('#deliveryFeedingDetailContainer').dxDataGrid('instance').getTotalSummaryValue('AmountTotal');
        ...
        if ($('#deliveryFeedingDetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#deliveryFeedingDetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                detailRecords = $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option("dataSource");
            });
        }
        else {
            detailRecords = $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option("dataSource");
        ...
        if (empr_DeliveryFeeding.rowsCount == detailRecords.length) {
            var modelRecord = {
                Master: masterRecord,
            ...
            var modelRecord = {
                Master: masterRecord,
                Detail: $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option("dataSource")
            };
            return modelRecord;
        }
    },
    GetGridData: async function () {
        return await $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option("dataSource");
    },
    ValidateInfo: function () {

        var valid = true;
        var data = empr_DeliveryFeeding.GetDataToSave();
        ...
        data.Detail = $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option("dataSource");
    ...
    SaveInfo: function () {
        debugger;
        var dataModel = empr_DeliveryFeeding.GetDataToSave();
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/DeliveryFeeding/Save", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                if (dataModel.Master.TRAN_ID == 0
                    || dataModel.Master.TRAN_ID == null
                ...
                //empr_DeliveryFeeding.GetDeliveryFeedingDetailsByCode($('#Code').val());
                //$('#BtnDelete').show();
                //empr_DeliveryFeeding.ResetForm();
                if (dataClear == 1) {
                    empr_DeliveryFeeding.GetDeliveryFeedingByCode($('#Code').val());
                }
                else {
                    empr_DeliveryFeeding.ResetForm();
                }
    ...
    GetDeliveryFeedingByCode: function (code) {
        ...
        ajaxHelper.ajaxGetJson('/DeliveryFeeding/GetDeliveryFeedingByCode?code=' + code, function (data) {
            console.log('data',data);
            if (data.master.msgType == 1) {
                ...
                if (masterData.length == 1) {
                    ...
                    empr_DeliveryFeeding.InitDropdownsWithValue(response.selleR_CODE, response.buyeR_CODE, response.brokeR_CODE, response.cond, response.sbF_TYPE, response.coB_CODE, response.curR_CODE, response.unit, response.sodA_TYPE);
                    if (Permissions != "Admin") {
                    ...
                    empr_DeliveryFeeding.toggleOperatorsOnEdit();
                }
                if (data.detail.msgType == 1) {
                    ...
                    empr_DeliveryFeeding.CreateGrid(data.detail.data);
    ...
    GetDeliveryFeedingDefaultOperators: function () {
        ajaxHelper.ajaxGetJson('/DeliveryFeeding/GetDeliveryFeedingDefaultOperators', function (data) {
            if (data.msgType == 1) {
    ...
    GetDeliveryFeedingDetailsByCode: function (code) {
        ajaxHelper.ajaxGetJson('/DeliveryFeeding/GetDeliveryFeedingDetailByCode?code=' + code, function (data) {
            if (data.msgType == 1) {
                empr_DeliveryFeeding.CreateGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    Delete: function () {
        ...
        swal({
            ...
            ajaxHelper.ajaxPostJsonData({ code: $('#Code').val() }, "/DeliveryFeeding/Delete", function (data) {
                empr_helper.notify(data.msg, data.msgType);
                if (data.msgType == 1) {
                    empr_DeliveryFeeding.ResetForm();
                    $('#BtnDelete').hide();
    ...
    InitSodaPickGrid: function () {
        ...
        if (data.Master.SDATE == "" || data.Master.SDATE == null || data.Master.SDATE == undefined) {
            ...
            empr_DeliveryFeeding.GetSodaBookFeedingDetailBySodaDate(data.Master.SDATE);
        }
    },
        ...
        ajaxHelper.ajaxGetJson('/DeliveryFeeding/GetSodaBookFeedingDetailBySodaDate?sodaDate=' + sodaDate, function (data) {
            ...
            if (data.msgType == 1) {
                if (data.data.length > 0) {
                    ...
                    empr_DeliveryFeeding.CreateSodaPickGrid(data.data);
        ...
        if ($('#SodaPickGridContainer').dxDataGrid('instance').hasEditData()) {
            $('#SodaPickGridContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                if (IsDataAvailableInGrid) {
                    var existingData = $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option('dataSource');
                    var selectedSodas = $('#SodaPickGridContainer').dxDataGrid('instance').getSelectedRowKeys();
                    console.log('1', selectedSodas);
                    selectedSodas = empr_DeliveryFeeding.SetData(selectedSodas);
                    var brokerValue = '';
                    if ($('#BrokerCode').data('dxSelectBox') != undefined) {
                        brokerValue = $('#BrokerCode').data('dxSelectBox').option('value');
                    }
                    if (selectedSodas.length > 0) {
                        empr_DeliveryFeeding.InitDropdownsWithValue(selectedSodas[0].selleR_CODE, selectedSodas[0].buyeR_CODE, selectedSodas[0].brokeR_CODE, selectedSodas[0].cond, selectedSodas[0].sbF_TYPE, selectedSodas[0].coB_CODE, selectedSodas[0].curR_CODE, selectedSodas[0].unit, selectedSodas[0].sodA_TYPE);
                    ...
                    $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option('dataSource', finalData);
                }
                else {
                    var selectedSodas = $('#SodaPickGridContainer').dxDataGrid('instance').getSelectedRowKeys();
                    selectedSodas = empr_DeliveryFeeding.SetData(selectedSodas);
                    ...
                    $('#deliveryFeedingDetailContainer').dxDataGrid('instance').option('dataSource', selectedSodas);
                }
                $('.modal').hide();
                $('#V_DATE').focus();
            });
    ...
    SetData: function (dataSource) {
        $.each(dataSource, function (index, item) {
            ...
            item.__KEY__ = empr_DeliveryFeeding.GenerateKey(36);
    ...
    InitDropdowns: function () {
        debugger;
        empr_DeliveryFeeding.InitConditionDDL();
        ajaxHelper.ajaxGetJson("/DeliveryFeeding/GetParties", function (data) {
            if (data.msgType == 1) {
                empr_DeliveryFeeding.InitSellerCodeDDL(data.data);
                empr_DeliveryFeeding.InitBuyerCodeDDL(data.data);
                empr_DeliveryFeeding.InitBrokerCodeDDL(data.data);
                empr_DeliveryFeeding.InitCoBrokerCodeDDL(data.data);
            }
            else {
    ...
    InitDropdownsWithValue: function (sCode, bCode, brCode, condition, sbfType, coBro, curr, unit, sodaType) {
        debugger;
        empr_DeliveryFeeding.InitConditionDDL(condition);
        ajaxHelper.ajaxGetJson("/DeliveryFeeding/GetParties", function (data) {
            if (data.msgType == 1) {
                empr_DeliveryFeeding.InitSellerCodeDDL(data.data, sCode);
                empr_DeliveryFeeding.InitBuyerCodeDDL(data.data, bCode);
                empr_DeliveryFeeding.InitBrokerCodeDDL(data.data, brCode);
                empr_DeliveryFeeding.InitCoBrokerCodeDDL(data.data, coBro);
                empr_DeliveryFeeding.InitCurrencyDDL(curr);
                empr_DeliveryFeeding.InitUnitDDL(unit);
                empr_DeliveryFeeding.InitSodaTypeDDL(sodaType);
                empr_DeliveryFeeding.InitTypeDDL(sbfType);
            }
            else {
                empr_helper.notify(data.data, data.msgType);
            }
    ...
    InitReportTypeDDL: function (selectedValue) {
        ajaxHelper.ajaxGetJson("/DeliveryFeeding/GetReportTypes", function (data) {
            if (data.msgType == 1) {
                if (data.data.length > 0) {
    ...
    GeneratePrintReport: function () {
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/DeliveryFeeding/GetPrintReport", function (data) {
            if (data.msgType == 1) {
                $('#ModalBody').empty();
                setTimeout(function () {
