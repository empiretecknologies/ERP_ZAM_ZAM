var empr_SodaBookFeeding = {
    ...
    InitEvents: function () {
        $(document).ready(function () {
            $('.coBroker').hide();
            empr_SodaBookFeeding.ResetForm();
            window.addEventListener('message', function (event) {
                ...
                if (data && data.traN_ID) {
                    ...
                    empr_SodaBookFeeding.GetSodaBookFeedingByCode(data.traN_ID);
                }
            });

            $('body').on('click', '#BtnQuickSearch', function () {
                empr_SodaBookFeeding.InitQuickSearchGrid();
            });
            ...
            $('body').on('click', '#BtnSave', function () {
                if (Permissions != "Admin") {
                    if (!$("#Code").val() && !Permissions.r_ADD) {
                        ...
                        if (empr_SodaBookFeeding.ValidateInfo()) {
                            empr_SodaBookFeeding.SaveInfo();
                        }
                    }
                } else {
                    if (empr_SodaBookFeeding.ValidateInfo()) {
            ...
            $('body').on('click', '.elm_edit', function () {
                ...
                $('#Code').val(id);
                $('.modal').modal('hide');
                empr_SodaBookFeeding.GetSodaBookFeedingByCode(id);
            });
            ...
            $('body').on('click', '#saveCopiedRecord', function () {
                ajaxHelper.ajaxPostJsonData({ traN_ID: empr_helper.selectedBill, v_DATE: $('#updatedDate').val() }, "/SodaBookFeeding/CopyRecord", function (data) {
                    console.log(data.data);
                    empr_helper.notify(data.msg, data.msgType);
                    if (data.msgType == 1) {
                        $('.modal').modal('hide');
                        empr_SodaBookFeeding.GetSodaBookFeedingByCode(data.data.code);
                    }
                }, false, true);
            });

            $('body').on('click', '#BtnDelete', function () {
                empr_SodaBookFeeding.Delete();
            });
    ...
    ResetForm: function () {
        empr_SodaBookFeeding.CreateGrid([{ __KEY__: empr_SodaBookFeeding.GenerateKey(36), chK1: true, chk: "1" }]);
        empr_SodaBookFeeding.InitTypeDDL();
        empr_SodaBookFeeding.InitPurchaseAndSaleDDL();
        empr_SodaBookFeeding.InitSodaTypeDDL();
        empr_SodaBookFeeding.InitCurrencyDDL();
        empr_SodaBookFeeding.InitUnitDDL();
        ...
        empr_SodaBookFeeding.InitDropdowns();
        $('#V_DATE').focus();
    },
    CreateGrid: function (dataSrc) {
        if (dataSrc.length > 0) {
            empr_SodaBookFeeding.rowsCount = dataSrc.length - 1;
        }
        var col = [
            {
                dataField: "Action",
                ...
                cellTemplate: function (container, options) {
                    ...
                    if (Permissions != "Admin") {
                        const copyAction = !Permissions.r_COPY
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Clone" onclick="empr_SodaBookFeeding.CloneRow(${options.rowIndex})" title="Duplicate"><i class="fa fa-clone"></i></a>`;
                        const addAction = (!Permissions.r_ADD && !Permissions.r_EDIT)
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Add" style="margin-left: 8px" onclick="empr_SodaBookFeeding.AddRow()" title="Add"><i class="fa fa-add"></i></a>`;
                        const deleteAction = !Permissions.r_DLT
                            ? ''
                            : `<a href="javascript:;" class="grid-action-icon Delete" style="margin-left: 8px" onclick="empr_SodaBookFeeding.DeleteRow(${options.rowIndex},${options.data.dT_CODE})" title="Delete"><i class="fa fa-trash"></i></a>`;
                        const actions = `<div class="btn-group btn-group-sm">${copyAction}${addAction}${deleteAction}</div>`;
                        $(actions).appendTo(container);
                    } else {
                        let extraAction = '';
                        if ($('#Code').val() != '') {
                            extraAction = `<a href="javascript:;" class="grid-action-icon" style="margin-left: 8px; color:#FFD700" onclick="empr_SodaBookFeeding.ShowCostCenterModal(${options.data.traN_ID},${options.data.dT_CODE},'${options.data.dT_DESC}',${options.data.amt})"><i class="fa fa-coins"></i></a>`;
                        }
                ...
                setCellValue: function (newData, value, currentRowData) {
                    ...
                    if (!isNaN(qty) && !isNaN(qtY2)) {
                        if (currentRowData.chK1) {
                            ...
                            if (empr_SodaBookFeeding.Branch_RT_TYPE == 'Y') {
                            ...
                            if (empr_SodaBookFeeding.Branch_RT_TYPE == 'N') {
            ...
            },
            {
                ...
                caption: 'RT Type',
                lookup: {
                    dataSource: empr_SodaBookFeeding.RT_TYPES,
                    ...
                    onCustomItemCreating: function (args) {
                        if (args.text != undefined) {
                            ...
                            if (isNaN(newItem.key)) {
                                ...
                                empr_SodaBookFeeding.RT_TYPES.insert(newItem);
    ...
    CloneRow: function (index) {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                empr_SodaBookFeeding.rowsCount += 1;
                ...
                if (dataSource.length > 0) {
                    ...
                    //delete clonedRowData.dT_CODE;
                    clonedRowData.__KEY__ = empr_SodaBookFeeding.GenerateKey(36);
    ...
    AddRow: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                dataSource.unshift({ __KEY__: empr_SodaBookFeeding.GenerateKey(36), chK1: true, chk: "1" });
                gridInstance.option("dataSource", dataSource);
            ...
            if (dataSource.length > 1) {
                ...
                if (dtCode == '' || dtCode == null || dtCode == undefined) {
                    ...
                    if (availableRows.length > 1) {
                        ...
                        ajaxHelper.ajaxPostJsonData({ code: dtCode }, "/SodaBookFeeding/DeleteSodaBookFeedingDetailByCode", function (data) {
                            empr_helper.notify(data.msg, data.msgType);
                            if (data.msgType == 1) {
                                gridInstance.deleteRow(index);
                                empr_SodaBookFeeding.rowsCount -= 1;
    ...
    GetDataToSave: function () {
        ...
        if ($('#DetailContainer').dxDataGrid('instance').hasEditData()) {
            $('#DetailContainer').dxDataGrid('instance').saveEditData().done(function () {
                ...
                //var response = empr_SodaBookFeeding.GetGridData();
                //response.then((data) => {
                //    debugger;
                //    console.log(data);
                //    var modelRecord = {
        ...
        if (empr_SodaBookFeeding.rowsCount == detailRecords.length) {
            var modelRecord = {
                Master: masterRecord,
                Detail: detailRecords
            };
    ...
    ValidateInfo: function () {
        ...
        var valid = true;
        var data = empr_SodaBookFeeding.GetDataToSave();
    ...
    SaveInfo: function () {
        debugger;
        var dataModel = empr_SodaBookFeeding.GetDataToSave();
        ...
        ajaxHelper.ajaxPostJsonData(dataModel, "/SodaBookFeeding/Save", function (data) {
            empr_helper.notify(data.msg, data.msgType);
            if (data.msgType == 1) {
                if (dataModel.Master.TRAN_ID == 0
                    || dataModel.Master.TRAN_ID == null
                ...
                if (dataClear == 1) {
                    empr_SodaBookFeeding.GetSodaBookFeedingDetailsByCode($('#Code').val());
                    $('#BtnDelete').show();
                }
                else {
                    empr_SodaBookFeeding.ResetForm();
    ...
    InitQuickSearchGrid: function () {
        empr_SodaBookFeeding.GetSodaBookFeedings();
    },
    GetSodaBookFeedings: function () {
        ajaxHelper.ajaxGetJson('/SodaBookFeeding/GetSodaBookFeedings', function (data) {
            if (data.msgType == 1) {
                empr_SodaBookFeeding.CreateQuickSearchGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    GetSodaBookFeedingByCode: function (code) {
        ajaxHelper.ajaxGetJson('/SodaBookFeeding/GetSodaBookFeedingByCode?code=' + code, function (data) {
            console.log('data ...............',data);
            if (data.master.msgType == 1) {
                ...
                if (masterData.length == 1) {
                    ...
                    empr_SodaBookFeeding.InitDropdownsWithValue(response.selleR_CODE, response.buyeR_CODE, response.brokeR_CODE, response.cond, response.sbF_TYPE, response.curR_CODE, response.unit, response.sodA_TYPE);
                ...
                if (data.detail.msgType == 1) {
                    empr_SodaBookFeeding.CreateGrid(data.detail.data);
    ...
    GetSodaBookFeedingDetailsByCode: function (code) {
        ajaxHelper.ajaxGetJson('/SodaBookFeeding/GetSodaBookFeedingDetailByCode?code=' + code, function (data) {
            if (data.msgType == 1) {
                empr_SodaBookFeeding.CreateGrid(data.data);
            }
            else {
                empr_helper.notify(data.msg, data.msgType);
            }
    ...
    Delete: function () {
        ...
        swal({
            ...
            ajaxHelper.ajaxPostJsonData({ code: $('#Code').val() }, "/SodaBookFeeding/Delete", function (data) {
                empr_helper.notify(data.msg, data.msgType);
                if (data.msgType == 1) {
                    empr_SodaBookFeeding.ResetForm();
                    $('#BtnDelete').hide();
    ...
    InitDropdowns: function () {
        debugger;
        empr_SodaBookFeeding.InitConditionDDL();
        ajaxHelper.ajaxGetJson("/SodaBookFeeding/GetParties", function (data) {
            if (data.msgType == 1) {
                empr_SodaBookFeeding.InitSellerCodeDDL(data.data);
                empr_SodaBookFeeding.InitBuyerCodeDDL(data.data);
                empr_SodaBookFeeding.InitBrokerCodeDDL(data.data);
                empr_SodaBookFeeding.InitCoBrokerCodeDDL(data.data);
            }
            else {
    ...
    InitDropdownsWithValue: function (sCode, bCode, brCode, condition, type, currCode, unit, sodaType) {
        debugger;
        empr_SodaBookFeeding.InitConditionDDL(condition);
        empr_SodaBookFeeding.InitCurrencyDDL(currCode);
        empr_SodaBookFeeding.InitUnitDDL(unit);
        empr_SodaBookFeeding.InitTypeDDL(type);
        empr_SodaBookFeeding.InitSodaTypeDDL(sodaType);
        ajaxHelper.ajaxGetJson("/SodaBookFeeding/GetParties", function (data) {
            if (data.msgType == 1) {
                empr_SodaBookFeeding.InitSellerCodeDDL(data.data, sCode);
                empr_SodaBookFeeding.InitBuyerCodeDDL(data.data, bCode);
                empr_SodaBookFeeding.InitBrokerCodeDDL(data.data, brCode);
            }
            else {
                empr_helper.notify(data.data, data.msgType);
            }
