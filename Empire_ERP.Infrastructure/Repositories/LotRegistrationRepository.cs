    ...
    public class LotRegistrationRepository : ILotRegistrationRepository
    {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.CODE == 0)
                            {
                                ...
                                query = "INSERT INTO " + table + " " +
                                            "(CODE,LOT_NO,V_DATE,PARTY_CODE,ACT_CODE,ADD_USER_ID,ADD_DATE," +
                                            ...
                                            "VALUES" +
                                            "('" + newCode + "','" + modelRecord.LOT_NO + "','" + modelRecord.V_DATE + "','" + modelRecord.PARTY_CODE + "','" + modelRecord.ACT_CODE + "','" + userid + "','" + CommonService.GetDateTime("Pakistan Standard Time") + "'," +
                                ...
                                Duplicationquery = "SELECT COUNT(*) FROM " + table + " WHERE LOT_NO = '" + modelRecord.LOT_NO + "' AND MENU_ID = '" + common.MenuID + "' AND DLT = 'T'";
                                ...
                                else
                                {
                                    ...
                                    response.msg = "Lot Number Already Exist !....";
                                    response.msgType = 2;
                                }
                            }
                            else
                            {
                                query = "UPDATE " + table + " SET LOT_NO = '" + modelRecord.LOT_NO + @"',
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                string connectionString = new SQLService().getconnstring();
                LotRegistration lotRegistration = new LotRegistration();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    while (reader.Read())
                    {
                        lotRegistration = new LotRegistration
                        {
                            CODE = 0,
                            V_DATE = record.V_DATE,
                            LOT_NO = record.ITEM_NAME,
                ...
                response = this.Save(lotRegistration, common);
        ...
        public MyHttpResponseMessage GetLotRegistrationById(int id, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = "SELECT CODE,LOT_NO,V_DATE,PARTY_CODE,ACT_CODE,ASTATUS " +
                                       "FROM " + table + " " +
