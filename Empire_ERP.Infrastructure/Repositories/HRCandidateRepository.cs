...
namespace Empire_ERP.Infrastructure.Repositories
{
    public class HRCandidateRepository : IHRCandidateRepository
    {
        ...
        public HRCandidateRepository(IMenuRepository menuRepository, IBranchRepository branchRepository, ICommonRepository commonRepository)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        string query = $@"SELECT CAN.TRAN_ID, CAN.V_DATE, J.JOB_TITLE , CAN.FULL_NAME, CAN.EMAIL, CAN.PHONE, CASE WHEN CAN.GENDER = 'M' THEN 'Male' ELSE 'Female' END AS GENDER, CAN.DOB, CAN.CON_ADDRESS, ED.GROUP_NAME AS EDUCATION, 
                                        CAN.EXP_YEAR, CAN.RESUME_PATH, CAN.APPLY_DATE,  CASE WHEN CAN.ASTATUS = 'Y' THEN 'Active' ELSE 'In-Active' END AS ASTATUS, CAN.REMARKS
                                        FROM {table} CAN
                                        LEFT OUTER JOIN TBL_JOB_TYPE J ON J.TRAN_ID = CAN.TRAN_ID
                                        LEFT OUTER JOIN TBL_EDUCATION ED ON ED.GROUP_CODE = CAN.EDUCATION_ID
                                        WHERE CAN.DLT = 'T' ORDER BY TRAN_ID DESC";
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                CON_ADDRESS = reader["CON_ADDRESS"].ToString(),
                                EDUCATION = reader["EDUCATION"].ToString(),
                                EXP_YEAR = Convert.ToDecimal(reader["EXP_YEAR"]).ToString("0.0") + " Years",
                                RESUME_PATH = reader["RESUME_PATH"].ToString(),
                                APPLY_DATE = reader["APPLY_DATE"] == DBNull.Value ? null : Convert.ToDateTime(reader["APPLY_DATE"]).ToString("yyyy-MM-dd"),
        ...
        public MyHttpResponseMessage Save(HRCandidate modelRecord, Core.Entities.Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.TRAN_ID == 0 || modelRecord.TRAN_ID is null)
                            {
                                ...
                                query = "INSERT INTO " + table + " " +
                                                "([TRAN_ID],[V_DATE], JOB_ID, FULL_NAME, EMAIL, PHONE, [GENDER], [DOB], [CON_ADDRESS], [EDUCATION_ID], [EXP_YEAR], [RESUME_PATH], [APPLY_DATE], [REMARKS], [ASTATUS]," +
                                                ...
                                                + modelRecord.EXP_YEAR + "','"
                                                + modelRecord.DOC + "','"
                                                + modelRecord.APPLY_DATE + "','"
                                                + modelRecord.REMARKS + "','"
                                                + modelRecord.ASTATUS + "','"
                                                + userid + "','"
                            ...
                            else
                            {
                                query = "UPDATE " + table + " SET " +
                                        ...
                                        "[JOB_ID] = '" + modelRecord.JOB + "', " +
                                        "[FULL_NAME] = '" + modelRecord.NAME + "', " +
                                        "[EMAIL] = '" + modelRecord.EMAIL + "', " +
                                        "[PHONE] = '" + modelRecord.PHONE + "', " +
                                        "[GENDER] = '" + modelRecord.GENDER + "', " +
                                        "[DOB] = '" + modelRecord.DOB + "', " +
                                        "[CON_ADDRESS] = '" + modelRecord.ADDRESS + "', " +
                                        "[EDUCATION_ID] = '" + modelRecord.EDUCATION + "', " +
                                        "[EXP_YEAR] = '" + modelRecord.EXP_YEAR + "', " +
                                        "[RESUME_PATH] = '" + modelRecord.DOC + "', " +
                                        "[APPLY_DATE] = '" + modelRecord.APPLY_DATE + "', " +
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        if (reader.Read())
                        {
                            var jsonDataResult = new
                            {
                                TRAN_ID = reader["TRAN_ID"].ToString(),
                                V_DATE = reader["V_DATE"] == DBNull.Value ? null : Convert.ToDateTime(reader["V_DATE"]).ToString("yyyy-MM-dd"),
                                JOB = Convert.ToInt32(reader["JOB_ID"]),
                                NAME = reader["FULL_NAME"].ToString(),
                                EMAIL = reader["EMAIL"].ToString(),
                                PHONE = reader["PHONE"].ToString(),
                                GENDER = reader["GENDER"].ToString(),
                                DOB = reader["DOB"] == DBNull.Value ? null : Convert.ToDateTime(reader["DOB"]).ToString("yyyy-MM-dd"),
                                ADDRESS = reader["CON_ADDRESS"].ToString(),
                                EDUCATION = Convert.ToInt32(reader["EDUCATION_ID"]),
                                EXP_YEAR = Convert.ToDecimal(reader["EXP_YEAR"]),
                                DOC = reader["RESUME_PATH"].ToString(),
                                APPLY_DATE = reader["APPLY_DATE"] == DBNull.Value ? null : Convert.ToDateTime(reader["APPLY_DATE"]).ToString("yyyy-MM-dd"),
                                ASTATUS = reader["ASTATUS"].ToString(),
                                REMARKS = reader["REMARKS"].ToString(),
                            };
