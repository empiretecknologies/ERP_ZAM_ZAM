...
{
    public class DocumentRetrievalRepository : IDocumentRetrievalRepository
    {
        public IMenuRepository _menuRepository { get; set; }
        public IBranchRepository _branchRepository { get; set; }

        public DocumentRetrievalRepository(IMenuRepository menuRepository, IBranchRepository branchRepository)
        {
					...
					using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
					{
                        if(search == "M")
                        {
                            string query = $@"SELECT M.TRAN_ID,M.V_DATE,M.VOUCHER_NO, M.REF, CASE WHEN M.ASTATUS = 'Y' THEN 'Active' ELSE 'In-Active' END AS ASTATUS ,M.REMARKS, M.ADD_USER_ID, 
                                                ...
                                                BM.TRAN_ID AS PTRAN_ID,BM.VOUCHER_NO AS PVOUCHER_NO, MB.ID AS MENU_ID ,MB.MENU_PAGE,MB.MENU_PARENT_CODE 
                                                FROM {table} M 
                                                LEFT OUTER JOIN TBL_DR_DETAIL D ON D.TRAN_ID = M.TRAN_ID 
                                                LEFT OUTER JOIN TBL_BD_DETAIL BD ON BD.DT_CODE = D.PICK_ID
                                                LEFT OUTER JOIN TBL_BD_MASTER BM ON BM.TRAN_ID = BD.TRAN_ID 
                                                LEFT OUTER JOIN TBL_MENU_BUILDER MB ON MB.ID = BM.MENU_ID
                        ...
                        else
                        {
                            string query = $@"SELECT M.TRAN_ID, M.V_DATE, M.VOUCHER_NO, M.REF, CASE WHEN M.ASTATUS = 'Y' THEN 'Active' ELSE 'In-Active' END AS ASTATUS, M.REMARKS, M.ADD_USER_ID, 
                                                M.ADD_DATE, M.ADD_COMPUTER_NAME, M.ADD_IP_ADDRESS, M.EDIT_USER_ID, M.EDIT_DATE, M.EDIT_COMPUTER_NAME, M.EDIT_IP_ADDRESS, M.ADD_POSTALCODE, M.EDIT_POSTALCODE,
                                                PM.VOUCHER_NO AS PICK_DATA, PM.TRAN_ID AS PTRAN_ID, MB.ID AS MENU_ID ,MB.MENU_PAGE,MB.MENU_PARENT_CODE, D.DT_DESC
                                                FROM {table} M 
        ...
        public MyHttpResponseMessage GetDocumentRetrievalDetailByCode(int code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"SELECT D.*, PM.VOUCHER_NO AS PICK_DATA, PM.TRAN_ID AS PTRAN_ID, MB.ID AS MENU_ID ,MB.MENU_PAGE,MB.MENU_PARENT_CODE FROM {table} D 
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                VOUCHER = Convert.ToString(reader["VOUCHER"]),
                                    ...
                                    if (item.DT_CODE == null || item.DT_CODE == 0)
                                    {
                                        ...
                                        if (detailCode > 0)
                                        {
                                            detailQuery = $@"INSERT INTO {detailTable} 
                                                            ([TRAN_ID], [DT_CODE], [VOUCHER], [SWIFT], [CURR_CODE], [CRATE], [DT_DESC], [BCODE], 
                                                             ...
                                                             [MENU_ID], [DLT], [PICK_ID]) 
                                                            VALUES ({modelRecord.Master.TRAN_ID}, {detailCode}, '{item.VOUCHER}', '{item.SWIFT}', 
                                                                    '{item.CURR_CODE}', '{item.CRATE}', '{item.DT_DESC}', '{branch}', '{period}', 
        ...
        public MyHttpResponseMessage GetSodaBookFeedingDetail(Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table) && !String.IsNullOrWhiteSpace(detailTable) && !String.IsNullOrWhiteSpace(pickMasterTable) && !String.IsNullOrWhiteSpace(pickDetailTable))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = @$"SELECT SD.DT_CODE ,SM.V_DATE, SM.VOUCHER_NO, SM.REF, SM.REMARKS, SD.ORIGNAL_DOC,
                                        SD.DHL_NO 
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                REF = Convert.ToString(reader["REF"]),
                                REMARKS = Convert.ToString(reader["REMARKS"]),
                                ORIGNAL_DOC = Convert.ToString(reader["ORIGNAL_DOC"]),
