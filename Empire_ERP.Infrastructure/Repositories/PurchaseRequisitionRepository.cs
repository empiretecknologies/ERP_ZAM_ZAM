...
namespace Empire_ERP.Infrastructure.Repositories
{
    public class PurchaseRequisitionRepository : IPurchaseRequisitionRepository
    {
        ...
        public PurchaseRequisitionRepository(IMenuRepository menuRepository, IBranchRepository branchRepository)
        {
				...
				if (!String.IsNullOrWhiteSpace(table))
				{
					...
					using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
					{
                        string query = "SELECT M.TRAN_ID,M.V_DATE,M.VOUCHER_NO," +
                                       "M.REF,M.REMARKS,W.DESCR AS WAREHOUSE,M.BCODE,M.PERIOD_ID," +
                                       ...
                                       "M.ADD_POSTALCODE,M.EDIT_POSTALCODE,M.ASTATUS " +
									   $"FROM {table} M " +
                                       $"LEFT OUTER JOIN TBL_WAREHOUSE W ON W.CODE = M.WAREHOUSE " +
        ...
        public MyHttpResponseMessage GetPurchaseRequisitionDetailByCode(int code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = "SELECT DT_CODE,ITEM_CODE, DEP,QTY," +
                        ...
                        try
                        {
                            ...
                            if (modelRecord.Master.TRAN_ID == null || modelRecord.Master.TRAN_ID == 0)
                            {
                                ...
                                query = $"INSERT INTO {table}" +
                                        "(TRAN_ID,V_DATE,VOUCHER_NO,WAREHOUSE," +
                                        ...
                                        "VALUES" +
                                        "('" + code + "','" + modelRecord.Master.V_DATE + "','" + voucherNo + "','" + modelRecord.Master.WAREHOUSE + "'," +
                                    ...
                                    if (item.DT_CODE == null || item.DT_CODE == 0)
                                    {
                                        ...
                                        if (detailCode > 0)
                                        {
                                            ...
                                            detailQuery = $"INSERT INTO {detailTable}" +
                                                           "(TRAN_ID,DT_CODE,ITEM_CODE, DEP, QTY," +
                                                           "UNIT,QTY2,BAL_QTY,DT_DESC," +
                                                           "COLOR,SIZE,GRADE,WAREHOUSE," +
                                                           ...
                                                           "MENU_ID,DLT,CHK)" +
                                                           "VALUES" +
                                                           "('" + modelRecord.Master.TRAN_ID + "','" + detailCode + "','" + item.ITEM_CODE + "','" + item.DEP + "','" + item.QTY + "'," +
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                List<PurchaseRequisitionDetail> PurchaseRequisitionDetailList = new List<PurchaseRequisitionDetail>();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    {
                        var row = new PurchaseRequisitionDetail
                        {
                            ...
                            DEP = Convert.ToInt32(detail_Reader["DEP"]),
                            CHK = detail_Reader["CHK"] == DBNull.Value ? 0 : Convert.ToInt32(detail_Reader["CHK"])
                        };
                        PurchaseRequisitionDetailList.Add(row);
                    }
                ...
                var customRequisition = new CustomPurchaseRequisition
                {
                    Master = purchaseRequisition,
                    Detail = PurchaseRequisitionDetailList
                };
        ...
        public MyHttpResponseMessage GetDataForReport(PurchaseRequisitionRDLCReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            PurchaseRequisitionRDLCReport masterData = new PurchaseRequisitionRDLCReport();
            CustomPurchaseRequisitionForPrintReport reportData = new CustomPurchaseRequisitionForPrintReport();
            var Menu = _menuRepository.GetMenu(common.MenuID);
            string? table = string.Empty, detailTable = string.Empty;
            if (Menu.data != null)
            {
            ...
            try
            {
                ...
                if (menuDetails.MD_ID == 42)
                {
                    query = $@"SELECT
                             M.V_DATE,M.VOUCHER_NO,
                             CASE WHEN M.ASTATUS = 'Y' THEN 'Active' Else
                             CASE WHEN M.ASTATUS = 'Y' THEN 'In.Active' ELSE 'N/A' END END AS ASTATUS,
                             AG.DESCR AS DEP_NAME,W.DESCR AS WAREHOUSE ,M.REMARKS,
