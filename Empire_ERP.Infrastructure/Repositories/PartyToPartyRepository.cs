...
{
    public class PartyToPartyRepository : IPartyToPartyRepository
    {
        public IBranchRepository _branchRepository { get; set; }
        public ICommonRepository _commonRepository { get; set; }
        public IMenuRepository _menuRepository { get; set; }
        public PartyToPartyRepository(IBranchRepository branchRepository, ICommonRepository commonRepository, IMenuRepository menuRepository)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    string query = "SELECT * FROM (" +
                                   ...
                                   "PT.PARTY_NAME AS PARTY_NAME, A.REMARKS, A.ADD_USER_ID, A.ADD_DATE, A.ADD_COMPUTER_NAME," +
        ...
        public MyHttpResponseMessage Save(List<PartyToParty> modelRecord, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        var partyInformation = partiesData.ToList().Where(p => p.customizedKey == modelRecord[0].PARTY_CODE).FirstOrDefault();
                                ...
                                if (item.TRAN_ID == null || item.TRAN_ID == 0)
                                {
                                    ...
                                    if (item.DT_CODE > 0)
                                    {
                                        query = $"INSERT INTO {table} " +
                                                "(TRAN_ID, V_DATE, VOUCHER_NO, DOC, CURR_CODE, CRATE, PARTY_CODE, ACT_CODE, REMARKS, BCODE, PERIOD_ID, " +
                                                ...
                                                "DLT, DT_CODE, TACT_CODE, TPARTY_CODE, DC_TYPE, AMT, CHQ_NO, CHQ_DATE, DT_DESC) " +
                                                $"VALUES " +
                                                $"('{code}', '{item.V_DATE}', '{voucherNo}', '{item.DOC}', '{item.CURR_CODE}', " +
                                                $"'{item.CRATE}', '{(partyInformation == null ? item.PARTY_CODE : partyInformation.key)}'," +
                                                $"'{(partyInformation == null ? item.ACT_CODE : partyInformation.accountCode)}', '{item.REMARKS}', '{branch}', " +
                                                ...
                                                $"'{item.TACT_CODE}', '{item.TPARTY_CODE}', '{item.DC_TYPE}', " +
                                ...
                                else
                                {
                                    if (item.DT_CODE == null || item.DT_CODE == 0)
                                    {
                                        ...
                                        if (item.DT_CODE > 0)
                                        {
                                            query = $"INSERT INTO {table} " +
                                                    ...
                                                    $"'{item.CRATE}', '{partyInformation.key}','{partyInformation.accountCode}', '{item.REMARKS}', '{branch}', " +
                                                    $"'{periodID}', '{userid}', '{CommonService.GetDateTime("Pakistan Standard Time")}', " +
                                                    $"'{computerName}', '{ip}', " +
                                                    $"'{userid}', '{CommonService.GetDateTime("Pakistan Standard Time")}', '{computerName}', " +
                                                    $"'{ip}', '{postalCode}', '{postalCode}', " +
                                    ...
                                    else
                                    {
                                        ...
                                        query = $"UPDATE {table} SET " +
                                                ...
                                                $"DLT = 'T', " +
                                                $"TACT_CODE = {item.TACT_CODE}, " +
                                                $"TPARTY_CODE = {item.TPARTY_CODE}, " +
        ...
        public MyHttpResponseMessage GetPartyToPartyByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetPartyToPartyDetailsByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    string query = "SELECT DT_CODE, TACT_CODE, TPARTY_CODE, DC_TYPE, " +
                    ...
                    while (reader.Read())
                    {
                        ...
                        {
                            DT_CODE = Convert.ToString(reader["DT_CODE"]),
                            TACT_CODE = Convert.ToString(reader["TACT_CODE"]),
                            TPARTY_CODE = Convert.ToString(reader["TPARTY_CODE"]),
                            CUSTOM_ACT_CODE = $"{Convert.ToString(reader["TPARTY_CODE"])}{Convert.ToString(reader["TACT_CODE"])}",
                            DC_TYPE = Convert.ToString(reader["DC_TYPE"]),
                            AMT = Convert.ToString(reader["AMT"]),
                            CHQ_NO = Convert.ToString(reader["CHQ_NO"]),
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                string connectionString = new SQLService().getconnstring();
                List<PartyToParty> PartyReceiptVoucherList = new List<PartyToParty>();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    while (reader.Read())
                    {
                        var row = new PartyToParty
                        {
                            ...
                            TACT_CODE = Convert.ToInt32(reader["TACT_CODE"]),
                            PARTY_CODE = Convert.ToString(reader["PARTY_CODE"]),
                            TPARTY_CODE = Convert.ToInt32(reader["TPARTY_CODE"]),
                            REMARKS = Convert.ToString(reader["REMARKS"]),
                            ASTATUS = Convert.ToString(reader["ASTATUS"]),
                            DOC = Convert.ToString(reader["DOC"]),
                            DT_CODE = 0,
                        ...
                        PartyReceiptVoucherList.Add(row);
                    }
                    reader.Close();
                    connection.Close();
                }
                response = this.Save(PartyReceiptVoucherList, common, menu);
        ...
        public MyHttpResponseMessage DeletePartyToPartyDetailByCode(int tranID, int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetDataForReport(CashReceiptRDLCReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            ...
            try
            {
                ...
                if (menuDetails.MD_ID == 39)
                {
                    ...
                    query = @$"SELECT 
                            PT.PARTY_NAME AS ACT_NAME ,A.DT_DESC AS DESCRIPTION1,
                            A.AMT
                            AS AMT,
                            CASE WHEN A.DC_TYPE = 'D' THEN
                            'Debit'
                            Else
                            'Credit'
                            End
                            ...
                            LEFT OUTER JOIN TBL_PARTY_TYPES PT
                            ON PT.ACT_CODE = A.TACT_CODE  AND PT.PARTY_CODE = A.TPARTY_CODE
                            Where  a.BCODE =  '{common.Branch}' And a.PERIOD_ID =  '{common.Period}'   AND A.DLT = 'T' AND A.TRAN_ID = '{modelRecord.TRAN_ID}'";
                }
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    if (menuDetails.MD_ID == 39)
                    {
                        while (reader.Read())
                        {
                            ...
                            dataRow["Type"] = reader["DC_TYPE"] == DBNull.Value ? "" : Convert.ToString(reader["DC_TYPE"]);
