...
{
    public class MerchantPurchaseOrderRepository : IMerchantPurchaseOrderRepository
    {
        public IMenuRepository _menuRepository { get; set; }
        public IBranchRepository _branchRepository { get; set; }
        public ICommonRepository _commonRepository { get; set; }
        public MerchantPurchaseOrderRepository(IMenuRepository menuRepository, IBranchRepository branchRepository, ICommonRepository commonRepository)
        {
        ...
        public MyHttpResponseMessage GetDeliveryFormats(Core.Entities.Common common)
        {
            ...
            try
            {
                List<MerchantPurchaseOrder> chartOfAccounts = new List<MerchantPurchaseOrder>();
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        while (reader.Read())
                        {
                            MerchantPurchaseOrder merchantPurchaseOrder = new MerchantPurchaseOrder();
                            merchantPurchaseOrder.ACT_CODE = Convert.ToInt32(reader["ACT_CODE"]);
                            //chartOfAccount.ACT_NAME = Convert.ToString(reader["ACT_NAME"]);
                            //chartOfAccount.ACT_SNAME = Convert.ToString(reader["CONRTOL_NAME"]);
                            chartOfAccounts.Add(merchantPurchaseOrder);
                        }
                        reader.Close();
                    }
        ...
        public MyHttpResponseMessage QuickSearch(Core.Entities.Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"SELECT MPO.*, CASE WHEN MPO.ASTATUS = 'Y' THEN 'Active' WHEN MPO.ASTATUS = 'N' THEN 'In-Active' ELSE '' END AASTATUS,
                                        CASE WHEN MPO.COMM_AMT = 'PR' THEN 'Percentage' WHEN MPO.COMM_AMT = 'RS' THEN 'Value' ELSE '' END COMM_TYPE,
                                        PTC.PARTY_NAME AS CLIENT_NAME,PTS.PARTY_NAME AS SUPPLIER_NAME,IT.ITEM_NAME,EMP.ENAME,PT.GROUP_NAME AS TERMS_NAME,
                                        U.GROUP_NAME AS UNIT_NAME,C.DESCR AS CURRENCY_NAME,D.DESCR , G.GROUP_NAME AS GRADE 
                                        FROM {table} MPO
                                        ...
                                        LEFT OUTER JOIN TBL_GSM GSM ON MPO.GSM = GSM.GROUP_CODE
                                        WHERE MPO.DLT = 'T' ORDER BY MPO.TRAN_ID DESC";
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                ASTATUS = reader["AASTATUS"].ToString(),
                                MENU_ID = reader["MENU_ID"].ToString(),
                                SPARTY_CODE = reader["SPARTY_CODE"].ToString(),
                                ...
                                TERMS_NAME = reader["TERMS_NAME"].ToString(),
                                UNIT_NAME = reader["UNIT_NAME"].ToString(),
                                GRADE_NAME = reader["GRADE"].ToString(),
                                COMM_TYPE = reader["COMM_TYPE"].ToString(),
        ...
        public MyHttpResponseMessage Save(MerchantPurchaseOrder modelRecord, Core.Entities.Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                //string checkQuery = $@"SELECT COUNT(*) FROM {table} WHERE CLIENT_PO = '{modelRecord.CLIENT_PO}'";
                                //SqlCommand checkCommand = new SqlCommand(checkQuery, connection, transaction);
                                //int count = (int)checkCommand.ExecuteScalar();

                                //if (count > 0)
                                //{
                                //    response.msg = "Client PO already exists!";
                                //    response.msgType = 2;
                                //    return response;
                                //}
                                ...
                                query = "INSERT INTO " + table + " " +
                                        "([TRAN_ID],[V_DATE],[VOUCHER_NO],[UNIT],[PARTY_CODE],[ACT_CODE],[REF],[JOB_NO],[SPARTY_CODE],[SACT_CODE]," +
                                        "[CLIENT_PO],[DEP_ID],[QTY],[RATE],[AMT],[CURR_CODE],[CRATE],[TERMS],[COMM_AMT],[COMM],[COMM_VAL],[REMARKS],[ITEM_CODE],[GRADE],[DOC],[BCODE],[PERIOD_ID]," +
                                        "[ADD_USER_ID],[ADD_DATE],[ADD_COMPUTER_NAME],[ADD_IP_ADDRESS],[ADD_POSTALCODE]," +
                                        "[EDIT_USER_ID],[EDIT_DATE],[EDIT_COMPUTER_NAME],[EDIT_IP_ADDRESS],[EDIT_POSTALCODE],[MENU_ID],[ASTATUS],[EMP_ID],[DLT],[DEP_TYPE])" +
                                        "VALUES " +
                                        "('" + GenerateNextId(common) + "','" + modelRecord.V_DATE + "','" + voucherNo + "','" + modelRecord.UNIT + "','" + modelRecord.PARTY_CODE + "','" + modelRecord.ACT_CODE + "'," +
                                        "'" + modelRecord.REF + "','" + jobNo + "','" + modelRecord.SPARTY_CODE + "','" + modelRecord.SACT_CODE + "','" + modelRecord.CLIENT_PO + "','" + modelRecord.DEP_ID + "'," +
                                        "'" + modelRecord.QTY + "','" + modelRecord.RATE + "','" + modelRecord.AMT + "','" + modelRecord.CURR_CODE + "','" + modelRecord.CRATE + "','" + modelRecord.TERMS + "','" + modelRecord.COMM_AMT + "','" + modelRecord.COMM + "','" + modelRecord.COMM_VAL + "'," +
                                        "'" + modelRecord.REMARKS + "','" + modelRecord.ITEM_CODE + "','" + modelRecord.GRADE + "','" + modelRecord.DOC + "','" + common.Branch + "','" + common.Period + "','" + userid + "','" + CommonService.GetDateTime("Pakistan Standard Time") + "'," +
                                        "'" + Computer + "','" + Ip + "','" + Postal + "','" + userid + "','" + CommonService.GetDateTime("Pakistan Standard Time") + "','" + Computer + "','" + Ip + "','" + Postal + "'," +
                                        "'" + common.MenuID + "','" + modelRecord.ASTATUS + "','" + modelRecord.EMP + "','T','" + depType + "')";
                            ...
                            else
                            {
                                ...
                                query = "UPDATE " + table + " SET " +
                                        ...
                                        [REF] = '" + modelRecord.REF + @"',
                                        [SPARTY_CODE] = '" + modelRecord.SPARTY_CODE + @"',
                                        [SACT_CODE] = '" + modelRecord.SACT_CODE + @"',
                                        [CLIENT_PO] = '" + modelRecord.CLIENT_PO + @"',
        ...
        public MyHttpResponseMessage GetMerchantPurchaseOrderById(int id, Core.Entities.Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        if (reader.Read())
                        {
                            var jsonDataResult = new
                            {
                                ...
                                //SHIP_DATE = reader["SHIP_DATE"] == DBNull.Value ? "" : Convert.ToDateTime(reader["SHIP_DATE"]).ToString("yyyy-MM-dd"),
                                REMARKS = reader["REMARKS"].ToString(),
                                BCODE = reader["BCODE"].ToString(),
                                PERIOD_ID = reader["PERIOD_ID"].ToString(),
                                ASTATUS = reader["ASTATUS"].ToString(),
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                string connectionString = new SQLService().getconnstring();
                MerchantPurchaseOrder DeliveryFormat = new MerchantPurchaseOrder();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (reader.Read())
                    {
                        DeliveryFormat = new MerchantPurchaseOrder
                        {
                            //TRAN_ID = 0,
        ...
        public MyHttpResponseMessage GetDataForReport(MerchantPurchaseOrderReport modelRecord, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            MerchantPurchaseOrderReport masterData = new MerchantPurchaseOrderReport();
            var Menu = _menuRepository.GetMenu(common.MenuID);
