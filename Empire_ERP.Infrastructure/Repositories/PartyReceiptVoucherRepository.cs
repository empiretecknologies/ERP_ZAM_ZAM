...
{
    public class PartyReceiptVoucherRepository : IPartyReceiptVoucherRepository
    {
        ...
        public PartyReceiptVoucherRepository(IBranchRepository branchRepository, ICommonRepository commonRepository, ICommonService commonService, IMenuRepository menuRepository)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    string query = "SELECT * FROM (" +
                                   ...
                                   "A.EDIT_IP_ADDRESS, A.ADD_POSTALCODE, A.EDIT_POSTALCODE, CASE WHEN A.ASTATUS = 'Y' THEN 'Active' WHEN A.ASTATUS = 'N' THEN 'In-Active' ELSE '' END ASTATUS,PT.PARTY_NAME, A.AMT, A.DT_DESC,A.CHQ_NO, A.CHQ_DATE," +
                                   "CASE WHEN A.DC_TYPE = 'D' THEN 'DEBIT' WHEN A.DC_TYPE = 'C' THEN 'CREDIT' ELSE '' END DC_TYPE," +
                    ...
                    while (reader.Read())
                    {
                        var row = new
                        {
                            ...
                            AMT = reader["AMT"] == DBNull.Value ? "0" : _commonService.ToAccountingFormat(Convert.ToDecimal(reader["AMT"])).ToString(),
        ...
        public MyHttpResponseMessage Save(List<PartyReceiptVoucher> modelRecord, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    try
                    {
                        ...
                        int code = 0;
                        PartyReceiptVoucher party = modelRecord.FirstOrDefault();
        ...
        public MyHttpResponseMessage GetPartyReceiptVoucherByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetPartyReceiptVoucherDetailsByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                string connectionString = new SQLService().getconnstring();
                List<PartyReceiptVoucher> PartyReceiptVoucherList = new List<PartyReceiptVoucher>();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    while (reader.Read())
                    {
                        var row = new PartyReceiptVoucher
                        {
                            TRAN_ID = 0,
        ...
        public MyHttpResponseMessage DeletePartyReceiptVoucherDetailByCode(int tranID, int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
