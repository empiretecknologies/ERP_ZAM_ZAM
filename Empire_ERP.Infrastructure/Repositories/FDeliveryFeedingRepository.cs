...
{
    public class FDeliveryFeedingRepository : IFDeliveryFeedingRepository
    {
        ...
        public FDeliveryFeedingRepository(IMenuRepository menuRepository, IBranchRepository branchRepository, IPartyRepository partyRepository, ICommonService commonService, ICommonRepository commonRepository)
        {
        ...
        public MyHttpResponseMessage QuickSearch(Common common)
        {
            ...
            try
            {
                ...
                string? pickMasterTable = string.Empty, pickDetailTable = string.Empty;
                if (Menu.data != null)
                {
                ...
                if (!String.IsNullOrWhiteSpace(table) && !String.IsNullOrWhiteSpace(detailTable))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = "SELECT A.TRAN_ID AS CODE, SBFM.TRAN_ID AS PTRAN_ID, SBFM.VOUCHER_NO AS SODA_VOUCHER,D.DT_CODE AS TRAN_ID, A.BGOD_CHARGES, A.SGOD_CHARGES, A.BLABOUR, A.SLABOUR, A.BFRIEGHT, A.SFRIEGHT, A.BFUMIGATION, A.SFUMIGATION, A.SBARDANA, A.BARDANA, A.BR_AMOUNT_BUYER, A.BR_AMOUNT_SELLER, A.WT_AMOUNT_SELLER, A.WT_AMOUNT_BUYER, A.S_NET_AMOUNT, A.B_NET_AMOUNT, A.V_DATE, A.VOUCHER_NO, A.DUE_DATE, " +
                                  ...
                                  "RTRIM(LTRIM(D.RT_TYPE)) + ' kg' as RT_TYPE, D.AMT, D.DT_DESC, MB.ID AS MENU_ID, MB.MENU_PAGE, MB.MENU_PARENT_CODE,CH.ACT_NAME AS COA " +
                                  $"FROM {table} A " +
                                  ...
                                  $"LEFT OUTER JOIN {pickDetailTable} SBFD ON D.PICK_ID = SBFD.DT_CODE AND SBFD.PERIOD_ID = D.PERIOD_ID AND SBFD.BCODE = D.BCODE " +
                                  $"LEFT OUTER JOIN {pickMasterTable} SBFM ON SBFD.TRAN_ID = SBFM.TRAN_ID AND SBFD.PERIOD_ID = SBFM.PERIOD_ID AND SBFD.BCODE = SBFM.BCODE " +
        ...
        public MyHttpResponseMessage Save(CustomFDeliveryFeeding modelRecord, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.Master.TRAN_ID == null || modelRecord.Master.TRAN_ID == 0)
                            {
                                ...
                                query = $"INSERT INTO {table}" +
                                        ...
                                        "BR_AMOUNT_BUYER_ST, BR_AMOUNT_SELLER_ST, COB_CODE, COB_ACODE, CURR_CODE, CRATE, SHIP_DATE, SHIP_STATUS, UNIT, SODA_TYPE, SBF_TYPE ,COA)" +
                                        "VALUES" +
                                        ...
                                        "'" + modelRecord.Master.COB_CODE + "','" + modelRecord.Master.COB_ACODE + "','" + modelRecord.Master.CURR_CODE + "','" + modelRecord.Master.CRATE + "','" + modelRecord.Master.SHIP_DATE + "','" + modelRecord.Master.SHIP_STATUS + "','" + modelRecord.Master.UNIT + "','" + modelRecord.Master.SODA_TYPE + "','" + modelRecord.Master.SBF_TYPE + "','" + modelRecord.Master.COA + "')";
        ...
        public MyHttpResponseMessage GetDeliveryFeedingDetailByCode(int code, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = @$"SELECT D.DT_CODE, D.ITEM_CODE, D.WAREHOUSE, D.QTY,D.UNIT, D.QTY2, D.BAL_QTY, D.RATE, D.CRATE, D.AMT, D.DT_DESC,D.TRUCK_NO, D.CONT_NO,D.SCOMP, D.CHK, D.PICK_ID,D.RT_TYPE, D.INS, D.LOT_NO
	                                        ,SBFM.VOUCHER_NO AS VOUCHER_NO,MB.ID AS MENU_ID ,MB.MENU_PAGE,MB.MENU_PARENT_CODE,SBFM.TRAN_ID,SBFM.BCODE,SBFM.PERIOD_ID FROM {table} D
                                            LEFT OUTER JOIN TBL_SBF_DETAIL SBF
                                            ON SBF.DT_CODE = D.PICK_ID AND SBF.PERIOD_ID = D.PERIOD_ID AND SBF.BCODE = D.BCODE
                                            LEFT OUTER JOIN TBL_SBF_MASTER SBFM
                                            ON SBF.TRAN_ID = SBFM.TRAN_ID AND SBF.PERIOD_ID = SBFM.PERIOD_ID AND SBF.BCODE = SBFM.BCODE
                                            LEFT OUTER JOIN TBL_MENU_BUILDER MB
                                            ON MB.ID = SBFM.MENU_ID
                                            WHERE
                                            D.DLT = 'T'
                                            AND D.TRAN_ID = '{code}'
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                FDeliveryFeeding deliveryFeeding = new FDeliveryFeeding();
                List<FDeliveryFeedingDetail> deliveryFeedingDetailList = new List<FDeliveryFeedingDetail>();
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (reader.Read())
                    {
                        deliveryFeeding = new FDeliveryFeeding
                        {
                    ...
                    {
                        var row = new FDeliveryFeedingDetail
                        {
                ...
                var customRequisition = new CustomFDeliveryFeeding
                {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table) && !String.IsNullOrWhiteSpace(detailTable) && !String.IsNullOrWhiteSpace(pickMasterTable) && !String.IsNullOrWhiteSpace(pickDetailTable))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        string query = @$"SELECT SM.TRAN_ID , SM.V_DATE, SM.VOUCHER_NO, UN.GROUP_NAME AS UNIT_NAME, PTS.PARTY_NAME AS SELLER, CH.ACT_NAME AS BUYER, CH.ACT_CODE AS COA_CODE, PTBR.PARTY_NAME AS BROKER
                                        ...
                                        AND SM.V_DATE = '{sodaDate}'
										AND SM.BCODE = '{common.Branch}' AND SM.PERIOD_ID = '{common.Period}' 
                                        ...
                                        GROUP BY
                                        SM.TRAN_ID , SM.V_DATE, SM.VOUCHER_NO,SM.REF,SM.REMARKS, PTS.PARTY_NAME ,CH.ACT_NAME , PTBR.PARTY_NAME 
                                        ...
                                        CRR.CODE,CRR.DESCR,SM.SODA_TYPE,SM.SHIP_DATE,SM.SHIP_STATUS,SM.COB_CODE,SM.COB_ACODE,COPT.PARTY_NAME,SM.SBF_TYPE,MB.ID ,MB.MENU_PAGE,MB.MENU_PARENT_CODE, CH.ACT_CODE";
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                COA_CODE = reader["COA_CODE"] == DBNull.Value ? 0 : Convert.ToInt32(reader["COA_CODE"]),
        ...
        public MyHttpResponseMessage GetDataForReport(FDeliveryFeedingReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
