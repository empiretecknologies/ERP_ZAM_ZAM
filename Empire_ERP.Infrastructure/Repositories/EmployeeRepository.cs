...
namespace Empire_ERP.Infrastructure.Repositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public EmployeeRepository(IMenuRepository menuRepository, ICommonRepository commonRepository, IPeriodRepository periodRepository)
        {
        ...
        public MyHttpResponseMessage Save(Employee modelRecord, Common common)
        {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(connectionString))
                    {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.EMP_CODE == 0)
                            {
                                modelRecord.EMP_CODE = Convert.ToInt32(GenerateNextId(common, pType));
                                query = $@"INSERT INTO {table} 
                                            ([EMP_CODE],[EMP_ID],[MACHINE_CODE],[ENAME],[DEP_ID],[FATHER_NAME],[DESIG],
                                            [GENDER],[BCODE],[SHIFT_T],[EMP_TYPE],[CELL_NO],[EMAIL],[REG],[OFFER_LETTER],[CHILD],
                                            ...
                                            '{modelRecord.GENDER}', '{modelRecord.BCODE}', '{modelRecord.SHIFT_T}', '{modelRecord.EMP_TYPE}', '{modelRecord.CELL_NO}', '{modelRecord.EMAIL}', '{modelRecord.REG}', '{modelRecord.OFFER_LETTER}', '{modelRecord.CHILD}', 
                                ...
                                Duplicationquery = $@"SELECT COUNT(*) FROM {table} WITH (NOLOCK) WHERE (EMP_ID = '{modelRecord.EMP_ID}' OR MACHINE_CODE = '{modelRecord.MACHINE_CODE}') AND BCODE = '{modelRecord.BCODE}' AND DLT = 'T'";
                                command.CommandText = Duplicationquery;
                                int count = (int)command.ExecuteScalar();
                                if (count == 1)
                                {
                                    transaction.Commit();
                                    response.data = modelRecord.EMP_CODE;
                                    response.msgType = 1;
                                ...
                                else
                                {
                                    ...
                                    response.msg = "The Employee ID Or Machine ID Already Exist !....";
                                    response.msgType = 2;
                                }
                            }
                            else
                            {
                                query = $@"UPDATE {table} SET 
                                              ...
                                              ,[REG] = '{modelRecord.REG}'
                                              ,[OFFER_LETTER] = '{modelRecord.OFFER_LETTER}'
                                ...
                                Duplicationquery = "SELECT COUNT(*) FROM " + table + " WITH (NOLOCK) WHERE ENAME = '" + modelRecord.ENAME + "' AND EMP_CODE != "+ modelRecord.EMP_CODE +" AND DLT = 'T'";
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"SELECT EM.EMP_CODE,EM.MACHINE_CODE,EM.EMP_IMG ,
                                      ...
                                      WHERE EM.DLT = 'T' ORDER BY EM.EMP_CODE DESC";
        ...
        public MyHttpResponseMessage GetEmployeeByEmployeeCode(int code, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = $@"SELECT [EMP_CODE], [EMP_ID], [MACHINE_CODE], [ENAME], [DEP_ID], [FATHER_NAME], [DESIG], [GENDER], [BCODE], [SHIFT_T]
                                    ...
                                    , [EDUCTION], [VEH_NUMBER], [LTYPE], [LNUMBER], [L_IDATE], [L_EDATE], [LEFT_DATE], [REASON_L], [OFFER_LETTER], [ADD_USER_ID]
                        ...
                        if (reader.Read())
                        {
                            var jsonDataResult = new
                            {
                                ...
                                ,OFFER_LETTER = reader["OFFER_LETTER"] == DBNull.Value ? 0 : Convert.ToInt32(reader["OFFER_LETTER"])
        ...
        public MyHttpResponseMessage GetEmployeeByEmployeeCode(int partyCode)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
