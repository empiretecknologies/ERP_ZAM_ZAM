...
{
    public class SetupTypeRepository : ISetupTypeRepository
    {
        public IMenuRepository _menuRepository { get; set; }
        public SetupTypeRepository(IMenuRepository menuRepository)
        {
        ...
        public MyHttpResponseMessage QuickSearch(Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    {
                        string query = "SELECT GROUP_CODE,GROUP_NAME,QTY,ADD_USER_ID," +
        ...
        public MyHttpResponseMessage Save(SetupType modelRecord, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.GROUP_CODE == null || modelRecord.GROUP_CODE == 0)
                            {
                                query = "INSERT INTO " + table + " " +
                                            "(GROUP_CODE,GROUP_NAME,QTY,ADD_USER_ID,ADD_DATE," +
                                            ...
                                            "VALUES" +
                                            "('" + GenerateNextId(common) + "','" + modelRecord.GROUP_NAME + "','" + modelRecord.QTY + "','" + userid + "','" + CommonService.GetDateTime("Pakistan Standard Time") + "'," +
        ...
        public MyHttpResponseMessage GetSetupTypeById(int id, Common common)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        string query = "SELECT GROUP_CODE,GROUP_NAME,ASTATUS,QTY " +
                                       "FROM " + table + " " +
                        ...
                        if (reader.Read())
                        {
                            var setupSubType = new SetupType
                            {
                                GROUP_CODE = Convert.ToInt32(reader["GROUP_CODE"]),
                                GROUP_NAME = Convert.ToString(reader["GROUP_NAME"]),
                                ASTATUS = Convert.ToString(reader["ASTATUS"]),
