...
{
    public class HRInterviewScheduleRepository : IHRInterviewScheduleRepository
	{
        public IMenuRepository _menuRepository { get; set; }
        public IBranchRepository _branchRepository { get; set; }
        public ICommonRepository _commonRepository { get; set; }
        public HRInterviewScheduleRepository(IMenuRepository menuRepository, IBranchRepository branchRepository, ICommonRepository commonRepository)
        {
            ...
            try
            {
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        string query = $@"SELECT INTS.TRAN_ID, CAN.FULL_NAME AS CAN_NAME, JOB.JOB_TITLE AS JOB, INTS.INT_DATE, INTS.INT_TIME, INTS.INT_LOCATION, INTS.INT_ROUND,
                                        INTS.EMP_EMAIL,  CASE WHEN INTS.ASTATUS = 'Y' THEN 'Active' ELSE 'In-Active' END AS ASTATUS, INTS.REMARKS
                                        FROM {table} INTS
                                        LEFT OUTER JOIN TBL_CANDIDATES CAN ON CAN.TRAN_ID = INTS.CON_ID
                                        LEFT OUTER JOIN TBL_JOB_TYPE JOB ON JOB.TRAN_ID = INTS.JOB_ID
                                        WHERE INTS.DLT = 'T' ORDER BY TRAN_ID DESC";
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
								...
								INT_DATE = reader["INT_DATE"] == DBNull.Value ? null : Convert.ToDateTime(reader["INT_DATE"]).ToString("yyyy-MM-dd"),
								INT_TIME = reader["INT_TIME"] == DBNull.Value ? null : ((TimeSpan)reader["INT_TIME"]).ToString(@"hh\:mm"),
								INT_LOCATION = reader["INT_LOCATION"].ToString(),
								INT_ROUND = Convert.ToInt32(reader["INT_ROUND"]),
								//CON_EMAIL = reader["CON_EMAIL"].ToString(),
        ...
        public MyHttpResponseMessage Save(HRInterviewSchedule modelRecord, Core.Entities.Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                        ...
                        try
                        {
                            ...
                            if (modelRecord.TRAN_ID == 0 || modelRecord.TRAN_ID is null)
                            {
                                ...
                                query = "INSERT INTO " + table + " " +
                                                "([TRAN_ID],CON_ID, JOB_ID, INT_DATE, INT_TIME, INT_LOCATION, INT_ROUND, [REMARKS], [ASTATUS], CON_EMAIL, EMP_ID, EMP_EMAIL," +
                                                ...
                                                + modelRecord.JOB + "','"
                                                + modelRecord.INT_DATE + "','"
                                                + modelRecord.INT_TIME + "','"
                                                + modelRecord.INT_LOCATION + "','"
                                                + modelRecord.INT_ROUND + "','"
                                                + modelRecord.REMARKS + "','"
                                                + modelRecord.ASTATUS + "','"
                                                + modelRecord.CAN_EMAIL + "','"
                                                + modelRecord.EMP + "','"
                                                + modelRecord.EMP_EMAIL + "','"
                                                + userid + "','"
                            ...
                            else
                            {
                                query = "UPDATE " + table + " SET " +
                                        "[CON_ID] = '" + modelRecord.CANDIDATE + "', " +
                                        "[JOB_ID] = '" + modelRecord.JOB + "', " +
										"[INT_DATE] = '" + modelRecord.INT_DATE + "', " +
										"[INT_TIME] = '" + modelRecord.INT_TIME + "', " +
										"[INT_LOCATION] = '" + modelRecord.INT_LOCATION + "', " +
										"[INT_ROUND] = '" + modelRecord.INT_ROUND + "', " +
                                        "[REMARKS] = '" + modelRecord.REMARKS + "', " +
                                        "[ASTATUS] = '" + modelRecord.ASTATUS + "', " +
										"[CON_EMAIL] = '" + modelRecord.CAN_EMAIL + "', " +
										"[EMP_ID] = '" + modelRecord.EMP + "', " +
										"[EMP_EMAIL] = '" + modelRecord.EMP_EMAIL + "', " +
