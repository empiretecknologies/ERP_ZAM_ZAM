...
{
    public class MpoTrimsRepository : IMpoTrimsRepository
    {
        public IBranchRepository _branchRepository { get; set; }
        public IMenuService _menuRepository { get; set; }
        public MpoTrimsRepository(IBranchRepository branchRepository, IMenuService menuRepository)
        {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    while (reader.Read())
                    {
                        var row = new
                        {
                            ...
                            //CARD_FILE_DATE = GetDate(reader["CARD_FILE_DATE"]),
                            //LAY_SUBM_DATE = GetDate(reader["LAY_SUBM_DATE"]),
                            //LAY_SUBD_DATE = GetDate(reader["LAY_SUBD_DATE"]),
                            //LAY_APPR_DATE = GetDate(reader["LAY_APPR_DATE"]),
                            //STOFF_SUBM_DATE = GetDate(reader["STOFF_SUBM_DATE"]),
                            //STOFF_SUBD_DATE = GetDate(reader["STOFF_SUBD_DATE"]),
                            //STOFF_APPR_DATE = GetDate(reader["STOFF_APPR_DATE"]),
        ...
        public MyHttpResponseMessage Save(CustomMpoTrims modelRecord, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                    ...
                    try
                    {
                        ...
                        foreach (var item in modelRecord.Detail.ToList())
                        {
                            try
                            {
                                ...
                                if (item.DT_CODE == null || item.DT_CODE == 0)
                                {
                                    ...
                                    if (detailCode > 0)
                                    {
                                        detailQuery = $@"INSERT INTO {detailTable}
                                                       (TRAN_ID, DT_CODE, SHEET_REC_DATE, SHEET_REC_DOC, PLAN_SUBM_DATE, ACT_SUBD_DATE, ACT_SUBD_DOC, ACT_APPR_DATE, 
                                                        LABEL_DATE, SNAP_DATE, INLAY_DATE, POLY_DATE, STIFNER_DATE, PLYBAG_DATE, CART_STKR_DATE, CART_INH_DATE,
                                                       ...
                                                       VALUES
                                                       ('{modelRecord.Master.TRAN_ID}', '{detailCode}', '{item.SHEET_REC_DATE}', '{item.SHEET_REC_DOC}', '{item.PLAN_SUBM_DATE}', 
                                                        '{item.ACT_SUBD_DATE}', '{item.ACT_SUBD_DOC}','{item.ACT_APPR_DATE}','{item.LABEL_DATE}', '{item.SNAP_DATE}', '{item.INLAY_DATE}', 
                                                        '{item.POLY_DATE}', '{item.STIFNER_DATE}', '{item.PLYBAG_DATE}', '{item.CART_STKR_DATE}', '{item.CART_INH_DATE}',
                                ...
                                {
                                    string revStatusCheckQuery = $@"SELECT REV_STATUS FROM TBL_MpoTrims_DETAIL WHERE DT_CODE = '{item.DT_CODE}'";
                                    ...
                                    detailQuery = $"UPDATE {detailTable} " +
                                                  $"SET SHEET_REC_DATE = '{item.SHEET_REC_DATE}', " +
                                                  $"SHEET_REC_DOC = '{item.SHEET_REC_DOC}', " +
                                                  $"PLAN_SUBM_DATE = '{item.PLAN_SUBM_DATE}', " +
                                                  $"ACT_SUBD_DATE = '{item.ACT_SUBD_DATE}', " +
                                                  $"ACT_SUBD_DOC = '{item.ACT_SUBD_DOC}', " +
                                                  $"ACT_APPR_DATE = '{item.ACT_APPR_DATE}', " +
                                                  $"LABEL_DATE = '{item.LABEL_DATE}', " +
                                                  $"SNAP_DATE = '{item.SNAP_DATE}', " +
                                                  $"INLAY_DATE = '{item.INLAY_DATE}', " +
                                                  $"POLY_DATE = '{item.POLY_DATE}', " +
                                                  $"STIFNER_DATE = '{item.STIFNER_DATE}', " +
                                                  $"PLYBAG_DATE = '{item.PLYBAG_DATE}', " +
                                                  $"CART_STKR_DATE = '{item.CART_STKR_DATE}', " +
                                                  $"CART_INH_DATE = '{item.CART_INH_DATE}', " +
        ...
        public MyHttpResponseMessage GetMpoTrimsByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetMpoTrimsDetailByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    ...
                    while (reader.Read())
                    {
                        var row = new
                        {
                            ...
                            SHEET_REC_DATE = GetDate(reader["SHEET_REC_DATE"], "MM"),
                            SHEET_REC_DOC = Convert.ToString(reader["SHEET_REC_DOC"]),
                            PLAN_SUBM_DATE = GetDate(reader["PLAN_SUBM_DATE"], "MM"),
                            ACT_SUBD_DATE = GetDate(reader["ACT_SUBD_DATE"], "MM"),
                            ACT_SUBD_DOC = Convert.ToString(reader["ACT_SUBD_DOC"]),
                            ACT_APPR_DATE = GetDate(reader["ACT_APPR_DATE"], "MM"),
                            LABEL_DATE = GetDate(reader["LABEL_DATE"], "MM"),
                            SNAP_DATE = GetDate(reader["SNAP_DATE"], "MM"),
                            INLAY_DATE = GetDate(reader["INLAY_DATE"], "MM"),
                            POLY_DATE = GetDate(reader["POLY_DATE"], "MM"),
                            STIFNER_DATE = GetDate(reader["STIFNER_DATE"], "MM"),
                            PLYBAG_DATE = GetDate(reader["PLYBAG_DATE"], "MM"),
                            CART_STKR_DATE = GetDate(reader["CART_STKR_DATE"], "MM"),
                            CART_INH_DATE = GetDate(reader["CART_INH_DATE"], "MM"),
        ...
        public MyHttpResponseMessage CopyRecord(CopyRecord record, Common common, Menu menu)
        {
            ...
            try
            {
                ...
                MpoTrims MpoTrims = new MpoTrims();
                List<MpoTrimsDetail> MpoTrimsDetailList = new List<MpoTrimsDetail>();
                ...
                var customMpoTrims = new CustomMpoTrims
                {
                    Master = MpoTrims,
                    Detail = MpoTrimsDetailList
                };

                response = this.Save(customMpoTrims, common, menu);
        ...
        public MyHttpResponseMessage DeleteMpoTrimsDetailByCode(int code, Common common, Menu menu)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
        ...
        public MyHttpResponseMessage GetDataForReport(MpoTrimsRDLCReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
