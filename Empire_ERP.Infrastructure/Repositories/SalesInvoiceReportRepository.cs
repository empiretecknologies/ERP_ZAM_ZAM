    ...
    public class SalesInvoiceReportRepository : ISalesInvoiceReportRepository
    {
        ...
        public MyHttpResponseMessage GetReportData(SalesInvoiceReport report, Common common)
        {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                {
                    if (report.ReportID < 24 || report.ReportID == 100 || report.ReportID == 140)
                    {
                        string query = $"EXEC SUMMARY_BARCODE '{report.ReportID}','{report.FromDate.Value.ToString("yyyy-MM-dd")}','{report.ToDate.Value.ToString("yyyy-MM-dd")}','{common.Branch}','{common.Period}','{report.Item}','{report.PartyCode}','{report.ActCode}','{report.Group}','{report.Category}','{report.SubCategory}','{report.Barcode}','{report.Size}','{report.Color}','{table1}','{table2}','{table3}','{table4}'";
                        SqlCommand command = new SqlCommand(query, connection);
                        connection.Open();
                        SqlDataReader reader = command.ExecuteReader();
                    ...
                    else if (report.ReportID == 74)
                    {
                        string query = @$"SELECT A.V_DATE, A.VOUCHER_NO as VOUCHER_NO,IT.ITEM_NAME AS ITEM_NAME,
                                          ...
                                          AND A.BCODE = '{common.Branch}' AND A.PERIOD_ID = '{common.Period}'
                    ...
                    //    string query = $@"EXEC STKPROC {report.ReportID},'{report.FromDate.Value.ToString("yyyy-MM-dd")}','{report.ToDate.Value.ToString("yyyy-MM-dd")}',{common.Branch},{common.Period},'',''";
                    ...
                    else
                    {
                        ...
                        else if (report.ReportID == 34 || report.ReportID == 146)
                        {
                            while (reader.Read())
                            {
                                var row = new CustomSalesInvoiceReport
                                {
                                    ...
                                    ItemId = reader["ITEM_ID"] == DBNull.Value ? "" : Convert.ToString(reader["ITEM_ID"]),
                                    Barcode = reader["BARCODE"] == DBNull.Value ? "" : Convert.ToString(reader["BARCODE"]),
                                    Qty = reader["BALANCE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["BALANCE"]),
                                    Rate = reader["PRICE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["PRICE"]),
                                    ShopPrice = reader["SHOP_PRICE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["SHOP_PRICE"]),
                                    Amt = (reader["BALANCE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["BALANCE"])) * (reader["PRICE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["PRICE"])),
                        ...
                        else if (report.ReportID == 89)
                        {
                            while (reader.Read())
                            {
                                var row = new CustomSalesInvoiceReport
                                {
                                    SNO = reader["S_NO"] == DBNull.Value ? 0 : Convert.ToInt32(reader["S_NO"]),
                                    ItemName = reader["ITEM_ID"] == DBNull.Value ? "" : Convert.ToString(reader["ITEM_ID"]),
                                    Category = reader["CATEGORY"] == DBNull.Value ? "" : Convert.ToString(reader["CATEGORY"]),
                                    SubCategory = reader["SUB_CATEGORY"] == DBNull.Value ? "" : Convert.ToString(reader["SUB_CATEGORY"]),
                                    Debit = reader["STOCK_IN"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["STOCK_IN"]),
                        ...
                        else if (report.ReportID == 148)
                        {
                            while (reader.Read())
                            {
                                decimal saleAmt = reader["SNET_AMT"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["SNET_AMT"]);
                                decimal purchaseAmt = reader["PAMT"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["PAMT"]);
                                decimal netProfit = saleAmt - purchaseAmt;

                                decimal profitLossPercent = saleAmt != 0 ? (netProfit / saleAmt) * 100 : 0;

                                if (netProfit < 0 && profitLossPercent > 0)
                                {
                                    profitLossPercent = -profitLossPercent;
                                }
                                else if (netProfit > 0 && profitLossPercent < 0)
                                {
                                    // Yeh case tab ho sakta hai jab saleAmt negative ho par return/refund ki wajah se net profit positive ho jaye
                                    profitLossPercent = Math.Abs(profitLossPercent);
                                }
                                ...
                                var row = new CustomSalesInvoiceReport
                                {
                                    ...
                                    Rate = reader["SRATE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["SRATE"]),
                                    NetAmt = saleAmt,
                                    PbRate = reader["PR_PRICE"] == DBNull.Value ? 0 : Convert.ToDecimal(reader["PR_PRICE"]),
                                    PAmt = purchaseAmt,
                                    LINK = "/" + Convert.ToString(reader["MENU_PAGE"]) + "?MOID=" + Convert.ToString(reader["MENU_PARENT_CODE"]) + "&Code=" + Convert.ToString(reader["MENU_ID"]),
                                    TRAN_ID = Convert.ToInt32(reader["TRAN_ID"]),
                                    NET_PROFIT1 = netProfit,

                                    Percentage =  Math.Round(profitLossPercent, 2) 
                                };
        ...
        public static void CalculateBalanceAmount(IEnumerable<CustomSalesInvoiceReport> jsonDataResult, int? reportId)
        {
            ...
            if (reportId == 88)
            {
                var distinctItems = jsonDataResult.Select(v => v.ItemName).Distinct();

                foreach (var item in distinctItems)
                {
                    var vouchers = jsonDataResult.Where(v => v.ItemName == item).ToList();
                    decimal runningBalance = 0;
