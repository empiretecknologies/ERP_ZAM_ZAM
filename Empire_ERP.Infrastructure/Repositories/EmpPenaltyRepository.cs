...
{
    public class EmpPenaltyRepository : IEmpPenaltyRepository
    {
        public IMenuRepository _menuRepository { get; set; }
        public EmpPenaltyRepository(IMenuRepository menuRepository)
        {
        ...
        public MyHttpResponseMessage Save(EmpPenalty modelRecord, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            try
            {
                                ...
                                try
                                {
                                    if (item.DT_CODE == null || item.DT_CODE == 0)
                                    {
                                        ...
                                        query = $@"INSERT INTO {table} 
                                                            ([TRAN_ID], [V_DATE], [PENALTY], [REASON], [PAMT], [BCODE], 
                                                            ...
                                                            VALUES ({detailCode}, '{item.date}', '{item.penalty}', '{item.reason}', '{item.pAmt}', 
                                                                    '{branch}', '{period}', 
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                PENALTY = reader["PENALTY"] != DBNull.Value ? Convert.ToString(reader["PENALTY"]) : "",
                                REASON = reader["REASON"] != DBNull.Value ? Convert.ToString(reader["REASON"]) : "",   // Fix here: was V_DLTOATE and incorrect column
                                PAMT = Convert.ToInt32(reader["PAMT"]) != 0 ? Convert.ToInt32(reader["PAMT"]) : 0,
