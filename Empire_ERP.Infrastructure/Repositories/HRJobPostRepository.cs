...
{
    public class HRJobPostRepository : IHRJobPostRepository
    {
        public IMenuRepository _menuRepository { get; set; }
        public IBranchRepository _branchRepository { get; set; }
        public ICommonRepository _commonRepository { get; set; }
        public HRJobPostRepository(IMenuRepository menuRepository, IBranchRepository branchRepository, ICommonRepository commonRepository)
        {
        ...
        public MyHttpResponseMessage GetHRJobPosts(Core.Entities.Common common)
        {
            ...
            try
            {
                List<HRJobPost> chartOfAccounts = new List<HRJobPost>();
                ...
                if (!String.IsNullOrWhiteSpace(table))
                {
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        while (reader.Read())
                        {
                            HRJobPost chartOfAccount = new HRJobPost();
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        while (reader.Read())
                        {
                            var row = new
                            {
                                ...
                                MIN_EXP = reader["MIN_EXP"].ToString(),
                                MAX_EXP = reader["MAX_EXP"].ToString(),
                                MIN_SALARY = reader["MIN_SALARY"].ToString(),
                                MAX_SALARY = reader["MAX_SALARY"].ToString(),
        ...
        //public MyHttpResponseMessage Save(HRJobPost modelRecord, Core.Entities.Common common)
        //{
        //    MyHttpResponseMessage response = new MyHttpResponseMessage();
        //    try
        //    {
        ...
        //                                        "([TRAN_ID],[V_DATE], JOB_TITLE, [DEP_ID], [JOB_TYPE], [JOB_LOCATION], [MIN_EXP], [MAX_EXP], [MIN_SALARY], [MAX_SALARY], [EDUCATION], [SKILL], [JOB_DESC], [JOB_RESP], [DUE_DATE], [WEB_PUBLISH], [BCODE], [EMP_SHARE], " +
        //                                        "[ADD_USER_ID],[ADD_DATE],[ADD_COMPUTER_NAME],[ADD_IP_ADDRESS],[ADD_POSTALCODE], " +
        //                                        "[EDIT_USER_ID],[EDIT_DATE],[EDIT_COMPUTER_NAME],[EDIT_IP_ADDRESS],[EDIT_POSTALCODE],[MENU_ID],[ASTATUS],[DLT],[DOC]) " +
        //                                        "VALUES " +
        //                                        "('" + modelRecord.TRAN_ID + "','"
        //                                        + modelRecord.V_DATE + "','"
        //                                        + modelRecord.JOB_TITLE + "','"
        //                                        + modelRecord.DEPARTMENT + "','"
        //                                        + modelRecord.JOB_TYPE + "','"
        //                                        + modelRecord.JOB_LOCATION + "','"
        //                                        + modelRecord.MINIMUM_EXPERIENCE + "','"
        //                                        + modelRecord.MAXIMUM_EXPERIENCE + "','"
        //                                        + modelRecord.MINIMUM_SALARY + "','"
        //                                        + modelRecord.MAXIMUM_SALARY + "','"
        //                                        + modelRecord.EDUCATION + "','"
        //                                        + modelRecord.SKILL + "','"
        //                                        + modelRecord.JOB_DESCRIPTION + "','"
        //                                        + modelRecord.JOB_RESPONSIBILITY + "','"
        //                                        + modelRecord.DUE_DATE + "','"
        //                                        + modelRecord.WEB_PUBLISH + "','"
        //                                        + modelRecord.BCODE + "','"
        //                                        + modelRecord.EMP_SHARE + "','"
        //                                        + userid + "','"
        ...
        //                                        + Postal + "','"
        //                                        + common.MenuID + "','"
        //                                        + modelRecord.ASTATUS + "','T','"
        //                                        + modelRecord.DOCUMENT + "')";
        ...
        //                                "[DEP_ID] = '" + modelRecord.DEPARTMENT + "', " +
        //                                "[JOB_TYPE] = '" + modelRecord.JOB_TYPE + "', " +
        //                                "[JOB_LOCATION] = '" + modelRecord.JOB_LOCATION + "', " +
        //                                "[MIN_EXP] = '" + modelRecord.MINIMUM_EXPERIENCE + "', " +
        //                                "[MAX_EXP] = '" + modelRecord.MAXIMUM_EXPERIENCE + "', " +
        //                                "[MIN_SALARY] = '" + modelRecord.MINIMUM_SALARY + "', " +
        //                                "[MAX_SALARY] = '" + modelRecord.MAXIMUM_SALARY + "', " +
        //                                "[EDUCATION] = '" + modelRecord.EDUCATION + "', " +
        //                                "[SKILL] = '" + modelRecord.SKILL + "', " +
        //                                "[JOB_DESC] = '" + modelRecord.JOB_DESCRIPTION + "', " +
        //                                "[JOB_RESP] = '" + modelRecord.JOB_RESPONSIBILITY + "', " +
        //                                "[DUE_DATE] = '" + modelRecord.DUE_DATE + "', " +
        //                                "[WEB_PUBLISH] = '" + modelRecord.WEB_PUBLISH + "', " +
        //                                "[BCODE] = '" + modelRecord.BCODE + "', " +
        //                                "[EMP_SHARE] = '" + modelRecord.EMP_SHARE + "', " +
        //                                "[EDIT_USER_ID] = '" + userid + "', " +
                        ...
                        try
                        {
                            if (modelRecord.TRAN_ID == 0 || modelRecord.TRAN_ID is null)
                            {
                                ...
                                command.CommandText = "INSERT INTO " + table + " " +
                                    "([TRAN_ID],[V_DATE], JOB_TITLE, [DEP_ID], [JOB_TYPE], [JOB_LOCATION], [MIN_EXP], [MAX_EXP], [MIN_SALARY], [MAX_SALARY], [EDUCATION], [SKILL], [JOB_DESC], [JOB_RESP], [DUE_DATE], [WEB_PUBLISH], [BCODE], " +
                                    "[ADD_USER_ID],[ADD_DATE],[ADD_COMPUTER_NAME],[ADD_IP_ADDRESS],[ADD_POSTALCODE], " +
                                    "[EDIT_USER_ID],[EDIT_DATE],[EDIT_COMPUTER_NAME],[EDIT_IP_ADDRESS],[EDIT_POSTALCODE],[MENU_ID],[ASTATUS],[DLT],[DOC]) " +
                                    "VALUES (@TRAN_ID, @V_DATE, @JOB_TITLE, @DEP_ID, @JOB_TYPE, @JOB_LOCATION, @MIN_EXP, @MAX_EXP, @MIN_SALARY, @MAX_SALARY, @EDUCATION, @SKILL, @JOB_DESC, @JOB_RESP, @DUE_DATE, @WEB_PUBLISH, @BCODE, " +
                                    "@ADD_USER_ID, @ADD_DATE, @ADD_COMPUTER_NAME, @ADD_IP_ADDRESS, @ADD_POSTALCODE, " +
                                    "@EDIT_USER_ID, @EDIT_DATE, @EDIT_COMPUTER_NAME, @EDIT_IP_ADDRESS, @EDIT_POSTALCODE, @MENU_ID, @ASTATUS, 'T', @DOC)";

                                command.Parameters.AddWithValue("@TRAN_ID", modelRecord.TRAN_ID);
                                command.Parameters.AddWithValue("@V_DATE", modelRecord.V_DATE);
                                command.Parameters.AddWithValue("@JOB_TITLE", modelRecord.JOB_TITLE);
                                command.Parameters.AddWithValue("@DEP_ID", modelRecord.DEPARTMENT);
                                command.Parameters.AddWithValue("@JOB_TYPE", modelRecord.JOB_TYPE);
                                command.Parameters.AddWithValue("@JOB_LOCATION", modelRecord.JOB_LOCATION);
                                command.Parameters.AddWithValue("@MIN_EXP", modelRecord.MINIMUM_EXPERIENCE);
                                command.Parameters.AddWithValue("@MAX_EXP", modelRecord.MAXIMUM_EXPERIENCE);
                                command.Parameters.AddWithValue("@MIN_SALARY", modelRecord.MINIMUM_SALARY);
                                command.Parameters.AddWithValue("@MAX_SALARY", modelRecord.MAXIMUM_SALARY);
                                command.Parameters.AddWithValue("@EDUCATION", modelRecord.EDUCATION);
                                command.Parameters.AddWithValue("@SKILL", modelRecord.SKILL);
                                command.Parameters.AddWithValue("@JOB_DESC", modelRecord.JOB_DESCRIPTION);
                                command.Parameters.AddWithValue("@JOB_RESP", modelRecord.JOB_RESPONSIBILITY);
                                command.Parameters.AddWithValue("@DUE_DATE", modelRecord.DUE_DATE);
                                command.Parameters.AddWithValue("@WEB_PUBLISH", modelRecord.WEB_PUBLISH);
                                command.Parameters.AddWithValue("@BCODE", modelRecord.BCODE);
                                //command.Parameters.AddWithValue("@EMP_SHARE", modelRecord.EMP_SHARE);
                                command.Parameters.AddWithValue("@ADD_USER_ID", userid);
                                command.Parameters.AddWithValue("@ADD_DATE", CommonService.GetDateTime("Pakistan Standard Time"));
                                command.Parameters.AddWithValue("@ADD_COMPUTER_NAME", Computer);
                                command.Parameters.AddWithValue("@ADD_IP_ADDRESS", Ip);
                                command.Parameters.AddWithValue("@ADD_POSTALCODE", Postal);
                                command.Parameters.AddWithValue("@EDIT_USER_ID", userid);
                                command.Parameters.AddWithValue("@EDIT_DATE", CommonService.GetDateTime("Pakistan Standard Time"));
                                command.Parameters.AddWithValue("@EDIT_COMPUTER_NAME", Computer);
                                command.Parameters.AddWithValue("@EDIT_IP_ADDRESS", Ip);
                                command.Parameters.AddWithValue("@EDIT_POSTALCODE", Postal);
                                command.Parameters.AddWithValue("@MENU_ID", common.MenuID);
                                command.Parameters.AddWithValue("@ASTATUS", modelRecord.ASTATUS);
                                command.Parameters.AddWithValue("@DOC", modelRecord.DOCUMENT);
                            ...
                            else
                            {
                                command.CommandText = "UPDATE " + table + " SET " +
                                        ...
                                        "[EDIT_DATE] = @EDIT_DATE, " +
                                        "[EDIT_COMPUTER_NAME] = @EDIT_COMPUTER_NAME, " +
                                        "[EDIT_IP_ADDRESS] = @EDIT_IP_ADDRESS, " +
                                        "[EDIT_POSTALCODE] = @EDIT_POSTALCODE, " +
                                        "[MENU_ID] = @MENU_ID, " +
                                        "[ASTATUS] = @ASTATUS, " +
                                        "[DOC] = @DOC " +
                    ...
                    using (SqlConnection connection = new SqlConnection(new SQLService().getconnstring()))
                    {
                        ...
                        if (reader.Read())
                        {
                            var jsonDataResult = new
                            {
                                ...
                                MIN_SALARY = reader["MIN_SALARY"].ToString(),
                                MAX_SALARY = reader["MAX_SALARY"].ToString(),
                                EDUCATION = Convert.ToInt32(reader["EDUCATION"]),
                                SKILL = reader["SKILL"].ToString(),
                                JOB_DESC = reader["JOB_DESC"].ToString(),
                                JOB_RESP = reader["JOB_RESP"].ToString(),
                                DUE_DATE = reader["DUE_DATE"] == DBNull.Value ? null : Convert.ToDateTime(reader["DUE_DATE"]).ToString("yyyy-MM-dd"),
                                WEB_PUBLISH = reader["WEB_PUBLISH"].ToString(),
                                BCODE = Convert.ToInt32(reader["BCODE"]),
                                EMP_SHARE = reader["EMP_SHARE"].ToString(),
                                DOC = reader["DOC"].ToString(),
                                STATUS = reader["ASTATUS"].ToString(),
                            };
                            json = jsonDataResult;
                        }
                        reader.Close();
        ...
        public MyHttpResponseMessage GetDataForReport(HRJobPostReport modelRecord, DataTable dataTable, CustomMenuDetail menuDetails, Company currentCompany, Common common)
        {
            MyHttpResponseMessage response = new MyHttpResponseMessage();
            HRJobPostReport masterData = new HRJobPostReport();
